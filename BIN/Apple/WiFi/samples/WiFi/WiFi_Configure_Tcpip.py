...
class WiFi_Configure_Tcpip(WiFiTest):
	...
	def setup(self):
		...
		if len(Config.accesspoints) == 1:
			self.ap = WiFiNetwork(Config.accesspoints[0])
		elif len(Config.accesspoints) > 1:
...
#			self.logger.Fail("The static TCP/IP setting assignment failed. The device's IP address is %s.", currentIP)
		...
		# If you want to check connectivity, ping www.apple.com to make sure we're connected.
		# Again, since this is a sample test, there's probably bogus IP information in the config.
