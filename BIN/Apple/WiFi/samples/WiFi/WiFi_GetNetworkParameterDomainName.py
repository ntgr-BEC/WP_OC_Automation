...
#
#  WiFi_GetNetworkParameterDomainName.py
#  WirelessAutomation
#
#  This test verifies GAS query for MCCMNC.
#
#  Created by Wayne Finlay on 05/29/2013.
...
class WiFi_GetNetworkParameterDomainName(WiFiTest):
	def __init__(self):
		super(WiFi_GetNetworkParameterDomainName, self).__init__()
	...
	def setup(self):
		...
		if len(Config.GAS) > 0:
			...
			if 'DomainNameList' in self.hs and 'bssid' in self.hs and 'channel' in self.hs:
	...
	def run(self):
		...
		result = self.dut.getWiFi().GetNetworkParameterDomainName(self.hs.bssid, self.hs.channel)
		if result:
			...
			for element in self.hs.DomainNameList:
			...
			self.logger.Fail(u"GAS Scan on {0} using channel {1} for NetworkParameterDomainName failed.".format(self.hs.bssid, self.hs.channel))
