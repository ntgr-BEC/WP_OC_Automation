...
#
#  WiFi_GetPrivateMACAddress.py
#  WirelessAutomation
#
#  This test will verify it was able to join and forget a network.
#
#  Created by Wayne Finlay on 04/14/2014.
...
class WiFi_GetPrivateMACAddress(WiFiTest):
	...
	def run(self):
		...
		if self.dut.getOS().Ping(pingTarget) == True:
			...
			privateMAC = self.dut.wifi.GetPrivateMACAddress()
			if privateMAC is not None:
				if not MacUtils.IsMACAddressEqual(privateMAC, "00:00:00:00:00:00"):
					self.logger.Pass("Private MAC Address is valid: %s" % privateMAC)
				else:
					self.logger.Fail("Private MAC Address should not be 0's")
			else:
				self.logger.Fail("Failed to get private MAC Address.")
		...
		if currentNetwork == None:
			...
			privateMAC = self.dut.wifi.GetPrivateMACAddress()
			if privateMAC is not None:
				if  MacUtils.IsMACAddressEqual(privateMAC, "00:00:00:00:00:00"):
					self.logger.Pass("Private MAC Address is valid for non-joined device: %s" % privateMAC)
				else:
					self.logger.Fail("Expecting private MAC Address to not be 00:00:00:00:00:00. Private MAC Address: %s" % privateMAC)
			else:
				self.logger.Fail("Failed to get private MAC Address.")
		else:
