...
class WiFi_MCCMNCQuery(WiFiTest):
	def __init__(self):
		super(WiFi_MCCMNCQuery, self).__init__()
	...
	def setup(self):
		...
		if len(Config.GAS) > 0:
			...
			if 'MCCMNCQuery' in self.hs and 'bssid' in self.hs and 'channel' in self.hs:
	...
	def run(self):
		...
		if result:
			...
			self.logger.Fail(u"GAS Scan on {0} using channel {1} for MCCMNC failed.".format(self.hs.bssid, self.hs.channel))
