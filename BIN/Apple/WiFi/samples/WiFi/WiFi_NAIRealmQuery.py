...
#
#  WiFi_NAIRealmQuery.py
#  WirelessAutomation
#
#  This test verifies GAS query for NAIRealm.
#
...
class WiFi_NAIRealmQuery(WiFiTest):
	def __init__(self):
		super(WiFi_NAIRealmQuery, self).__init__()
	...
	def setup(self):
		...
		if len(Config.GAS) > 0:
			...
			if 'NAIRealmList' in self.hs and 'bssid' in self.hs and 'channel' in self.hs:
	...
	def run(self):
		...
		if result:
			...
			self.logger.Fail(u"GAS Scan on {0} using channel {1} for NAIRealm failed.".format(self.hs.bssid, self.hs.channel))
