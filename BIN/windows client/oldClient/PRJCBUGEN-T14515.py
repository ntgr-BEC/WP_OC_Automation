    ...
    def test_cpa(self):
        ...
        if self.expectresult == "enable":
            if url_titleName != self.checkpoint:
                test_result = 0
                print ("Expect can open the url:%s , And the Expect title was:%s . But real title was: %s ."%(self.urlValue,self.checkpoint,url_titleName))
                self.capturescreen ()
        else:
            if url_titleName == self.checkpoint:
                test_result = 0
                print ("Expect can't open the url:%s , And the Expect title was:%s . But real title was: %s ."%(self.urlValue,UnopenError,url_titleName))
                self.capturescreen ()
