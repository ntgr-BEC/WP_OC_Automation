################################################################################
#
#  File Name    : IGMP-007.tcl
#
#  Description  : This testcase used to test whether DUT could remove no response
#                 snooping port from multicast table when max response expire.
#
#  Revision     :
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "IGMP-007.tcl" "******************** Starting Test case IGMP-007.tcl ********************"
...
sleep 300; # Wait for max response to let multicast table age out
set TxRx4 [UALReportPortRate $ntgrTG $ntgrPort4]
set TxRx5 [UALReportPortRate $ntgrTG $ntgrPort5]
set TxRx6 [UALReportPortRate $ntgrTG $ntgrPort6]
set rx5 [expr [lindex $TxRx5 1]/[lindex $TxRx4 0]]
...
if {$bRet == 1} {
    set NtgrTestResult(IGMP-007.tcl) "OK"
} else {
    set NtgrTestResult(IGMP-007.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "IGMP-007.tcl" "******************** Complete Test case IGMP-007.tcl ********************"
#*************************  End of Test case  **********************************