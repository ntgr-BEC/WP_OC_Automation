################################################################################
#
#  File Name    : L2ACL-047.tcl
#
#  Description  : This testcase used to test filtering function of layer2 ACL.
#                 Apply/No apply ACL to a port repeatedly, test ACL work correctly.
#
#  Revision     :
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "L2ACL-047.tcl" "******************** Starting Test case L2ACL-047.tcl ********************"
...
set bRet 1
NtgrDumpLog  $NTGR_LOG_TERSE  "L2ACL-047.tcl" "Repeat $ntgrRepeatTimes times to apply/no apply ACL operation"
for {set times 0} {$times<$ntgrRepeatTimes} {incr times} {
    ...
    CALInterfaceMACACLDisable $ntgrDUT $mac_acl_interface_list
}

if { $bRet == 1 } {
    set NtgrTestResult(L2ACL-047.tcl) "OK"
} else {
    set NtgrTestResult(L2ACL-047.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "L2ACL-047.tcl" "******************** Complete Test case L2ACL-047.tcl ********************"
#*************************  End of Test case  **********************************