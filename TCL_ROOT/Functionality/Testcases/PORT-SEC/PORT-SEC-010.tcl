################################################################################
#
#  File Name    : PORT-SEC-010.tcl
#
#  Description  : This testcase used to test deleting static port security by
#                 setting max limit to 0 and corresponding traffic will be
#                 discarded on the security port.
#
#  Revision     :
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-010.tcl" "******************** Starting Test case PORT-SEC-010.tcl ********************"
## Initialize the DUTs
...
CALSetMaxStatic $ntgrDUT1 $ntgrSecurityPort 0
set TxRx4 [UALReportPortRate $ntgrTG $ntgrPort4]
set TxRx6 [UALReportPortRate $ntgrTG $ntgrPort6]
set rx4 [expr [lindex $TxRx4 1]/[lindex $TxRx6 0]]
if {$rx4<0.05} {
...
if {$bRet == 1} {
    set NtgrTestResult(PORT-SEC-010.tcl) "OK"
} else {
    set NtgrTestResult(PORT-SEC-010.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-010.tcl" "******************** Complete Test case PORT-SEC-010.tcl ********************"
#*************************  End of Test case  **********************************