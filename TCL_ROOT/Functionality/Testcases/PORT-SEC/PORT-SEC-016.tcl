################################################################################
#
#  File Name    : PORT-SEC-016.tcl
#
#  Description  : This testcase used to test script file generation and re-applied
#
#  Revision     :
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-016.tcl" "******************** Starting Test case PORT-SEC-016.tcl ********************"
## Initialize the DUTs
...
CALSetMaxDynamic $ntgrDUT1 $lag_if 100
...
if {$rx4>0.45 && $rx4<0.55} {
    ...
    NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-016.tcl" "Unexpected: [lindex $TxRx4 1]/[lindex $TxRx6 0]=$rx4 should between 0.45 and 0.55"
}

CALDeleteScriptFile $ntgrDUT1
CALGenerateScriptFile $ntgrDUT1
CALResetConfiguration $ntgrDUT1
sleep 60
CALApplyScriptFile $ntgrDUT1
sleep 30
...
if {$bRet == 1} {
    set NtgrTestResult(PORT-SEC-016.tcl) "OK"
} else {
    set NtgrTestResult(PORT-SEC-016.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-016.tcl" "******************** Complete Test case PORT-SEC-016.tcl ********************"
#*************************  End of Test case  **********************************