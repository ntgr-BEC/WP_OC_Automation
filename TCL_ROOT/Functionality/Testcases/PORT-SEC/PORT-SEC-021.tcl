################################################################################
#
#  File Name    : PORT-SEC-021.tcl
#
#  Description  : This testcase used to test whether MAC security on physical
#                 port could work correctly while shutdown/no shutdown security port.
#
#  Revision     :
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-021.tcl" "******************** Starting Test case PORT-SEC-021.tcl ********************"
## Initialize the DUTs
...
if {$rx4>0.45 && $rx4<0.55} {
    ...
    NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-021.tcl" "Unexpected: [lindex $TxRx4 1]/[lindex $TxRx6 0]=$rx4 should between 0.45 and 0.55"
}
...
if {$bRet == 1} {
    set NtgrTestResult(PORT-SEC-021.tcl) "OK"
} else {
    set NtgrTestResult(PORT-SEC-021.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-021.tcl" "******************** Complete Test case PORT-SEC-021.tcl ********************"
#*************************  End of Test case  **********************************