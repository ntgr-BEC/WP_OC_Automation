################################################################################
#
#  File Name    : PORT-SEC-022.tcl
#
#  Description  : This testcase used to test whether MAC security LAG
#                 port could work correctly while shutdown/no shutdown LAG or
#                 its member ports.
#
#  Revision     :
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-022.tcl" "******************** Starting Test case PORT-SEC-022.tcl ********************"
## Initialize the DUTs
...
if {$rx4>0.45 && $rx4<0.55} {
    ...
    NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-022.tcl" "Unexpected: [lindex $TxRx4 1]/[lindex $TxRx6 0]=$rx4 should between 0.45 and 0.55"
}

## Shutdown/No shutdown the LAG
CALPortLinkDown $ntgrDUT1 $lag_if
CALPortLinkUp $ntgrDUT1 $lag_if
sleep 5
...
## Shutdown/No shutdown one of member ports of the LAG
foreach pt $ntgrLAGMemberPorts {
...
## Shutdown/No shutdown all member ports of the LAG
CALPortLinkDown $ntgrDUT1 $ntgrLAGMemberPorts
CALPortLinkUp $ntgrDUT1 $ntgrLAGMemberPorts
sleep 5
...
if {$bRet == 1} {
    set NtgrTestResult(PORT-SEC-022.tcl) "OK"
} else {
    set NtgrTestResult(PORT-SEC-022.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-022.tcl" "******************** Complete Test case PORT-SEC-022.tcl ********************"
#*************************  End of Test case  **********************************