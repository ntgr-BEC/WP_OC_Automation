################################################################################
#
#  File Name    : PORT-SEC-031.tcl
#
#  Description  : This testcase used to repeatedly enable/disable port security
#                 at global configuration mode to test stability.
#
#  Revision     :
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-031.tcl" "******************** Starting Test case PORT-SEC-031.tcl ********************"
## Initialize the DUTs
...
if {$rx4>0.59 && $rx4<0.61} {
    ...
    NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-031.tcl" "Unexpected: [lindex $TxRx4 1]/[lindex $TxRx6 0]=$rx4"
}

CALConfigStaticAddr $ntgrDUT1 $lag_if "move"
for {set i 0} {$i<$ntgrRepeatTimes} {incr i} {
    CALDisablePortSecurity $ntgrDUT1
    set TxRx4 [UALReportPortRate $ntgrTG $ntgrPort4]
    set TxRx6 [UALReportPortRate $ntgrTG $ntgrPort6]
    set rx4 [expr [lindex $TxRx4 1]/[lindex $TxRx6 0]]
...
if {$bRet == 1} {
    set NtgrTestResult(PORT-SEC-031.tcl) "OK"
} else {
    set NtgrTestResult(PORT-SEC-031.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-031.tcl" "******************** Complete Test case PORT-SEC-031.tcl ********************"
#*************************  End of Test case  **********************************