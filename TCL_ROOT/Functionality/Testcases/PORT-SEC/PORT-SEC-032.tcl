################################################################################
#
#  File Name    : PORT-SEC-032.tcl
#
#  Description  : This testcase used to repeatedly move dynamic address to static,
#                 then delete the static to test port security stability.
#
#  Revision     :
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-032.tcl" "******************** Starting Test case PORT-SEC-032.tcl ********************"
## Initialize the DUTs
...
for {set i 0} {$i<$ntgrRepeatTimes} {incr i} {
    ...
    if {$rx4>0.59 && $rx4<0.61} {
        ...
        NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-032.tcl" "Unexpected: [lindex $TxRx4 1]/[lindex $TxRx6 0]=$rx4"
    }
...
if {$bRet == 1} {
    set NtgrTestResult(PORT-SEC-032.tcl) "OK"
} else {
    set NtgrTestResult(PORT-SEC-032.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "PORT-SEC-032.tcl" "******************** Complete Test case PORT-SEC-032.tcl ********************"
#*************************  End of Test case  **********************************