################################################################################
#
#  File Name    : QOS-031.tcl
#
#  Description  : This testcase used to test stability of repeatedly
...
#*************************  Start of Test case  ********************************
NtgrDumpLog  $NTGR_LOG_TERSE  "QOS-031.tcl" "******************** Starting Test case QOS-031.tcl ********************"
...
for {set t 1} {$t<=$ntgrRepeatTimes} {incr t} {
    ...
    CALQosDisableDiffService $ntgrDUT1
    sleep 5
    set TxRx4 [UALReportPortRate $ntgrTG $ntgrPort4]
    set TxRx6 [UALReportPortRate $ntgrTG $ntgrPort6]
    set rx6 [expr [lindex $TxRx6 1]/[lindex $TxRx4 0]]
    ...
    CALQosEnableDiffService $ntgrDUT1
    sleep 5
}
...
if {$bRet == 1} {
    set NtgrTestResult(QOS-031.tcl) "OK"
} else {
    set NtgrTestResult(QOS-031.tcl) "NG"
}

NtgrDumpLog  $NTGR_LOG_TERSE  "QOS-031.tcl" "******************** Complete Test case QOS-031.tcl ********************"
#*************************  End of Test case  **********************************