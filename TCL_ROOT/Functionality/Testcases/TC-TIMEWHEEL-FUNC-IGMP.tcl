...
NtgrDumpLog $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-IGMP.tcl" "******************** Starting Test case TC-TIMEWHEEL-FUNC-IGMP.tcl ********************"
NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-IGMP.tcl" "******************** Starting Test case TC-TIMEWHEEL-FUNC-IGMP.tcl ********************"
set cycleInterval [getTimeWheelCycleInterval]
set totalNoOfCycle [getTimeWheelTotalNoOfCycle ]
NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-IGMP.tcl" "Cycle Interval : $cycleInterval"
NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-IGMP.tcl" "Total Number of Cycles : $totalNoOfCycle"

set cycle 1
set clockNextCycle [expr  [clock seconds ] + $cycleInterval ]
while { $cycle <= $totalNoOfCycle  || $totalNoOfCycle == 0 } { 
	NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-IGMP.tcl" "Started Cycle : $cycle"
	...
	foreach scriptId $scriptIds {
		...
		if { $cycle >= $scrStartCycle && ( $cycle <= $scrStopCycle || $scrStopCycle == 0 ) && \
		   [expr ( $cycle - $scrStartCycle ) % $scrRepeatInterval ] == 0  } {
			set scriptName [getTimeWheelScriptName $scriptId ]
			set cfgFileName [getTimeWheelScriptCfg $scriptId ]
			NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-IGMP.tcl" "Executing script $scriptName with Config File $cfgFileName"
			if { $cfgFileName != {} } {
	...
	NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-IGMP.tcl" "Testing result of cycle $cycle:"
...
NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-IGMP.tcl" "Completed Test Script TC-TIMEWHEEL-FUNC-IGMP.tcl"
