...
NtgrDumpLog $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-PORT.tcl" "******************** Starting Test case TC-TIMEWHEEL-FUNC-PORT.tcl ********************"
NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-PORT.tcl" "******************** Starting Test case TC-TIMEWHEEL-FUNC-PORT.tcl ********************"
set cycleInterval [getTimeWheelCycleInterval]
set totalNoOfCycle [getTimeWheelTotalNoOfCycle ]
NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-PORT.tcl" "Cycle Interval : $cycleInterval"
NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-PORT.tcl" "Total Number of Cycles : $totalNoOfCycle"

set cycle 1
set clockNextCycle [expr  [clock seconds ] + $cycleInterval ]
while { $cycle <= $totalNoOfCycle  || $totalNoOfCycle == 0 } { 
	NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-PORT.tcl" "Started Cycle : $cycle"
	...
	foreach scriptId $scriptIds {
		...
		if { $cycle >= $scrStartCycle && ( $cycle <= $scrStopCycle || $scrStopCycle == 0 ) && \
		   [expr ( $cycle - $scrStartCycle ) % $scrRepeatInterval ] == 0  } {
			set scriptName [getTimeWheelScriptName $scriptId ]
			set cfgFileName [getTimeWheelScriptCfg $scriptId ]
			NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-PORT.tcl" "Executing script $scriptName with Config File $cfgFileName"
			if { $cfgFileName != {} } {
	...
	NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-PORT.tcl" "Testing result of cycle $cycle:"
...
NtgrDumpLog  $NTGR_LOG_TERSE  "TC-TIMEWHEEL-FUNC-PORT.tcl" "Completed Test Script TC-TIMEWHEEL-FUNC-PORT.tcl"
