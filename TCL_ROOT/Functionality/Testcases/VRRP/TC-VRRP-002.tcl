###################################################################################################################
#
#  File Name	    	: TC-VRRP-002.tcl
#
#  Description     	:
#         This TCL tests configures port channel on switches.
#
#  Config file       : TC-VRRP-002.cfg
#
...
Netgear_log_file "TC-VRRP-002.tcl" "******** Starting Test case TC-VRRP-002.tcl **********"

foreach switch_name $SW_LIST {
	Netgear_log_file "TC-VRRP-002.tcl" "Clear current configuration on switch $switch_name and configure some initial info"
...
foreach {switch_name portlist} [array get ntgr_swDownPortList_002] {
...
Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration of VLAN"

foreach vlan_index $VLAN_index_list {
	Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration of VLAN $vlan_index"
	CALCreateVlan $vlan_index
}

Netgear_log_file "TC-VRRP-002.tcl" "Started routing on the Switches"
...
Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration of VRRP"
...
foreach switch_name $switch_VRRP_list {
	...
	Netgear_log_file "TC-VRRP-002.tcl" "Started enable VRRP on switch $switch_name"
	CALVRRPEnable $switch_name
	
	Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration vrid on switch $switch_name"
	CALAddVRRPVrid $switch_name $interface_list $vrid_list
	
	Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration vrip on switch $switch_name"
	CALAddVRRPVip $switch_name $interface_list $vrid_list $vip_list  
	
	Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration Priority on switch $switch_name"
	CALAddVrrpPriority $switch_name $interface_list $vrid_list $pri_list
	
	Netgear_disconnect_switch $switch_name
}

Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration of RIP"

foreach switch_name $switch_RIP_list {
    
	Netgear_connect_switch $switch_name 
	
	Netgear_log_file "TC-VRRP-002.tcl" "Enable RIP on switch $switch_name"
	set flag_enable [getRipGlobalStatus $switch_name]
	CALRipEnableSwitch $switch_name $flag_enable
	    
	Netgear_log_file "TC-VRRP-002.tcl" "Enable RIP on logical or physical interface of switch $switch_name"
	set interface_list [getRipInterface $switch_name]
	CALRipEnableInterface $switch_name $interface_list
	 
	Netgear_log_file "TC-VRRP-002.tcl" "Configure route RIP distributelist on switch $switch_name"	
...
Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration of TRAFFIC"

for_array_keys chassis_id ntgr_trafficTestInfo {
	set duration [keylget ntgr_trafficTestInfo($chassis_id) TG_TRAFFIC_DURATION]
	Netgear_log_file "TC-VRRP-002.tcl" "Connecting to Traffic Generator $chassis_id"
...
for_array_keys chassis_id ntgr_trafficTestInfo {
	...
	Netgear_log_file "TC-VRRP-002.tcl" "Loading the configuration on ports"
...
for_array_keys chassis_id ntgr_trafficTestInfo {
	...
	set port_list [keylget ntgr_trafficTestInfo($chassis_id) TG_PORT_LIST]
	Netgear_log_file "TC-VRRP-002.tcl" "Starting traffic on ports"
	foreach port $port_list {
...
for_array_keys chassis_id ntgr_trafficTestInfo {
	...
	set port_list [keylget ntgr_trafficTestInfo($chassis_id) TG_PORT_LIST]
	NtgrDumpLog $NTGR_LOG_TERSE   "TC-VRRP-002.tcl" "checking the rate on ports"
...
if {$loss > 5} {
	Netgear_log_file  "TC-VRRP-002.tcl" "Receive rate is not equal to the send rate."   
}
...
foreach switch_name $switch_VRRP_list {
	...
	Netgear_log_file "TC-VRRP-002.tcl" "Started Configuration Timer advertised on switch $switch_name"
	CALAddVrrpAdvertiseTimer $switch_name $interface_list $vrid_list $advertise_timer
...
if {$result == 1} {Netgear_log_file "TC-VRRP-002.tcl" "*******TC-VRRP-002.tcl passed********************"} 

Netgear_log_file "TC-VRRP-002.tcl" "******************** Completed Test case TC-VRRP-002.tcl ********************"
