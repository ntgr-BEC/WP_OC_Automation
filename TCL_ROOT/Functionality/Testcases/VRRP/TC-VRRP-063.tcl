###################################################################################################################
#
#  File Name	    	: TC-VRRP-063.tcl
#
#  Description     	:
#         This TCL tests configures port channel on switches.
#
#  Config file       : TC-VRRP-063.cfg
#
...
Netgear_log_file "TC-VRRP-063.tcl" "******** Starting Test case TC-VRRP-063.tcl **********"

foreach switch_name $SW_LIST {
    Netgear_log_file "TC-VRRP-063.tcl" "Clear current configuration on switch $switch_name and configure some initial info"
    CALResetConfiguration $switch_name
    CALSetSwitchPrompt $switch_name
}
#################################################################################################
#Netgear_log_file "TC-VRRP-063.tcl" "Started Configuration of LAG"
#
#for_array_keys channel_name ntgr_poChanList {
#	Netgear_log_file "TC-VRRP-063.tcl" "Configure port channel($channel_name) on each switch(es)."
#	CALCreatePortChannel $channel_name
#}
#################################################################################################
Netgear_log_file "TC-VRRP-063.tcl" "Started to modify TOPO"

foreach {switch_name portlist} [array get ntgr_swDownPortList_063] {
    Netgear_log_file "TC-VRRP-063.tcl" "Setting ports' status to down on switch $switch_name"
...
Netgear_log_file "TC-VRRP-063.tcl" "Started Configuration of VLAN"

foreach vlan_index $VLAN_index_list {
	Netgear_log_file "TC-VRRP-063.tcl" "Started Configuration of VLAN $vlan_index"
	CALCreateVlan $vlan_index
}

Netgear_log_file "TC-VRRP-063.tcl" "Started routing on the Switches"
...
##################################################################################################
Netgear_log_file "TC-VRRP-063.tcl" "Started Configuration of VRRP"
...
foreach switch_name $VRRP_SW {
   ...
   Netgear_log_file "TC-VRRP-063.tcl" "Started enable VRRP on switch $switch_name"
...
############################################################################################
Netgear_log_file "TC-VRRP-063.tcl" "Started Configuration of RIP"

foreach switch_name $switch_RIP_list {
    
    Netgear_connect_switch $switch_name 
    
    Netgear_log_file "TC-VRRP-063.tcl" "Enable RIP on switch $switch_name"
    set flag_enable [getRipGlobalStatus $switch_name]
    CALRipEnableSwitch $switch_name $flag_enable
    	
    Netgear_log_file "TC-VRRP-063.tcl" "Enable RIP on logical or physical interface of switch $switch_name"
    set interface_list [getRipInterface $switch_name]
    CALRipEnableInterface $switch_name $interface_list
         
    Netgear_log_file "TC-VRRP-063.tcl" "Configure route RIP distributelist on switch $switch_name"	
...
#########################################Configure MSTP####################################################
Netgear_log_file "TC-VRRP-063.tcl" "Started Configuration of MSTP"
...
Netgear_log_file "TC-VRRP-063.tcl" "Started Configuration of TRAFFIC"

for_array_keys chassis_id ntgr_trafficTestInfo {
	set duration [keylget ntgr_trafficTestInfo($chassis_id) TG_TRAFFIC_DURATION]
	NtgrDumpLog $NTGR_LOG_TERSE   "TC-VRRP-063.tcl" "Connecting to Traffic Generator $chassis_id"
...
for_array_keys chassis_id ntgr_trafficTestInfo {
	...
	NtgrDumpLog $NTGR_LOG_TERSE   "TC-VRRP-063.tcl" "Loading the configuration on ports"
...
for_array_keys chassis_id ntgr_trafficTestInfo {
	...
	set port_list [keylget ntgr_trafficTestInfo($chassis_id) TG_PORT_LIST]
	NtgrDumpLog $NTGR_LOG_TERSE   "TC-VRRP-063.tcl" "Starting traffic on ports"
	foreach port $port_list {
...
for_array_keys chassis_id ntgr_trafficTestInfo {
    ...
    set port_list [keylget ntgr_trafficTestInfo($chassis_id) TG_PORT_LIST]
    NtgrDumpLog $NTGR_LOG_TERSE   "TC-VRRP-063.tcl" "checking the rate on ports"
    set ratelist ""
    foreach port $port_list {
  ...
  set tr_tp [lindex $first_value 0]
	...
	if {$loss > 5} {
	NtgrDumpLog $NTGR_LOG_TERSE   "TC-VRRP-063.tcl" "Receive rate is not equal to the send rate."   
	}
...
foreach switch_name $SW_LIST {
    ...
    if {$result != 1} {puts "Switch $switch_name has rebooted in testing, TC-VRRP-063.tcl failed."}
}
    
Netgear_log_file "TC-VRRP-063.tcl" "******************** Completed Test case TC-VRRP-063.tcl ********************"
