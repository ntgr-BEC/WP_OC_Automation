...
set TestSuitxmlFile "./target/surefire-reports/TEST-TestSuite.xml"
...
proc ParseXMLFile {} {
    ...
    if {[file exists $TestSuitxmlFile] && [file exists $ConfigxmlFile]} {
        ...
        for {set i 1} {$i > 0} {incr i} {
            set node_classname "/testsuite/testcase\[$i\]/@classname"
            set filtureNode "/testsuite/testcase\[$i\]/failure/@type"
            set classname [$Tsdoc selectNodes string($node_classname)]
            if {$classname == ""} {
                break
            }
            
            set node_casename "/testsuite/testcase\[$i\]/@name"
            set casename [$Tsdoc selectNodes string($node_casename)]
            if {$casename != "test"} {
                continue
            }
            
            set time_name  "/testsuite/testcase\[$i\]/@time"
            set time [$Tsdoc selectNodes string($time_name)]
            if {$time == "0.0"} {
                continue
            }

            set filtureFlag [$Tsdoc selectNodes string($filtureNode)]

            set splitList [split $classname "."]
            set lsize [llength $splitList]
            set functionName [lindex $splitList [expr $lsize-3]]
            set topfunctionName [lindex $splitList [expr $lsize-4]]
            if {$topfunctionName != "Switch" && $topfunctionName != "webportal"} {
                set functionName $topfunctionName-$functionName
            }
            set CaseName [lindex $splitList [expr $lsize-2]]
