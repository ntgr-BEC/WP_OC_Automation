 ...
 *
 */
public class ExportJiraCaeForQTP {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ExportJiraCaeForQTP qtp = new ExportJiraCaeForQTP();
        qtp.createAllTestCaseExcel();
        qtp.createFunctionalityTestCaseExcel();
        qtp.createRunVBS();
        qtp.copyfile();
    ...
    public void createAllTestCaseExcel() throws IOException {
        ...
        String excelPath = JiraParam.ipAddress + "\\AUTOMATION\\Functionality\\" + jiraApi.testPlan + ".xlsx";
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                if (jiraAPI.RunCaseStatus.contains(testCaseStatus)) {
                    ...
                    if (autoamtion.equals("Yes")) {
                        ...
                        boolean containKey = !(caseName.contains("CLI") || caseName.contains("GUI"));
                        boolean GUIcase = containKey || caseName.contains("GUI");
                        boolean CLIcase = caseName.contains("CLI");
                        if (jiraAPI.configBy.equals("GUI")) {
                            if (GUIcase) {
    ...
    public void markExecuteCaseList(String objective, String caseID) throws IOException {
        OutputStream out = null;
        JiraAPI jiraApi = new JiraAPI();
        String excelPath = JiraParam.ipAddress + "\\AUTOMATION\\Functionality\\" + objective + "\\ExecuteCaseList.xls";
        File finalXlsxFile = new File(excelPath);
        Workbook workBook = getWorkbok(finalXlsxFile);
        Sheet sheet = workBook.getSheetAt(0);
    ...
    public void createFunctionalityTestCaseExcel() throws IOException {
        ...
        for (int rownum = 1; rownum <= sheet.getLastRowNum(); rownum++) {
            ...
            String caseName = row.getCell(3).toString().substring(0, 3);
    ...
    public void initExecuteCaseList(String objective) throws IOException {
        ...
        Workbook workBook = getWorkbok(finalXlsxFile);
        Sheet sheet = workBook.getSheetAt(0);
        System.out.println("excel init in " + objective);
        for (int rownum = 1; rownum <= sheet.getLastRowNum(); rownum++) {
            if (sheet.getRow(rownum) != null) {
                Row row = sheet.getRow(rownum);
    ...
    public void createRunVBS() throws IOException {
        ...
        Set<String> set = new HashSet<String>();
        List<String> vbsList = new ArrayList<String>();
        for (String object : ExcelList) {
            if (set.add(object)) {
                vbsList.add(object);
            }
        }
        String vbsPath = JiraParam.ipAddress + "\\AUTOMATION\\TESTSUITE\\0RUN-TEST-SET.vbs";
        ...
        writeInvbs("LogSystem_Switch_Type = qtpApp.Test.LastRunResults.DataTable.GlobalSheet.GetParameter(\"LogSystem_Switch_Type\")");
    ...
    public void copyfile() {
        if (JiraParam.useIP.contains(".")) {
            File source = new File("C:\\AUTOMATION\\CONFIG\\config_jira.xml");
            File dest = new File(JiraParam.ipAddress + "\\AUTOMATION\\CONFIG\\config_jira.xml");
            try {
