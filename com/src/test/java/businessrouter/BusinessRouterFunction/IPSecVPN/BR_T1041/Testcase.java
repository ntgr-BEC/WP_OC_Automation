package businessrouter.BusinessRouterFunction.IPSecVPN.BR_T1041;
...
public class Testcase extends TestCaseBase implements Config {
    ...
    public String sTestStr = "BR_T1041";
    public String sCurrentValue;
    public String sExpectedtValue;
    // public String sOldSW = WebportalParam.sw1IPaddress;

    @Feature("IPSec VPN") // 必须要加，对应目录名
    @Story("BR_T1041") // 对应testrunkey
    @Description("008-Check IPSec log") // 对应用例名字
    @TmsLink("1514024") // 对应用例详情页的URL最后几位数字
    ...
    @Step("Test Step 5:Check that DUT IPsec VPN log is correct")
    public void step5() {
        BrAdvancedLogsPage BrAdvancedLogsPage = new BrAdvancedLogsPage();
        BrAdvancedLogsPage.OpenLogsPage();
        MyCommonAPIs.sleepi(5);
        boolean result = BrAdvancedLogsPage.checkLogoutput("[IPSec VPN] CHILD_SA conn0 established with SPIs")
                && BrAdvancedLogsPage.checkLogoutput("[IPSec VPN] IKE_SA conn0[1] established between");
        if (result == true ) {
            micResult =  true;
            assertTrue(micResult,"Pass:  DUT IPsec VPN log is correct."  );             
        } else {
            micResult =  false;
            assertTrue(micResult,"Failed: DUT IPsec VPN log is incorrect.");
        }
     }
