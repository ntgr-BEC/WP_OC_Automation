...
public class TestCase extends TestCaseBase implements Config {
    ...
    public String sTestStr = "BR_T1071";
    public String sCurrentValue;
    public String sExpectedtValue;
    // public String sOldSW = WebportalParam.sw1IPaddress;

    @Feature("IPSec VPN") // 必须要加，对应目录名
    @Story("BR_T1071") // 对应testrunkey
    @Description("060-local subnet and remote sunnet config to single ip") // 对应用例名字
    @TmsLink("1517415") // 对应用例详情页的URL最后几位数字
    ...
    public void step5() {
        ...
        Result0 =  Brtmspage.CheckPingPassOrFailed(TmsNomacthICMPCommands);
        //Brtmspage.CloseTMSWindows();
        if (Result0 == false ) {
            micResult =  true;
            assertTrue(micResult,"Pass: ICMP taffic that doesn't match IPsec policy can't be forward by  VPN tunnel."  );             
        } else {
            micResult =  false;
            assertTrue(micResult,"Failed:ICMP taffic that doesn't match IPsec policy can be forward by  VPN tunnel.");
        }
        TmsPageHandle2 = Brtmspage.LoginTMS(WebportalParam.brlanclientip);
        Result0 =  Brtmspage.CheckPingPassOrFailed(TmsICMP2Commands);
        //Brtmspage.CloseTMSWindows();
        if (Result0 == true ) {
            micResult =  true;
            assertTrue(micResult,"Pass: ICMP taffic can be forward by  VPN tunnel."  );             
