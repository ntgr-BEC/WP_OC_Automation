package businessrouter.BusinessRouterFunction.Security_Schedule.BR_T69;
...
public class TestCase extends TestCaseBase implements Config {
    ...
    public String sTestStr = "BR_T69";
    public String sCurrentValue;
    public String sExpectedtValue;

    @Feature("Business Router.Security_Schedule") // 必须要加，对应目录名
    @Story("BR_T69") // 对应testrunkey
    @Description("001-Schedule - All Day") // 对应用例名字
    @TmsLink("1455072") // 对应用例详情页的URL最后几位数字
    ...
    @Step("Test Step 2: Add a schedule with  all day, Binding the schedule to a Block site/service item.")
    public void step2() {
        ...
        BrAdvancedSecuritySchedulePage.selectAllDayandEveryDay();
        boolean result1 = BrAdvancedSecuritySchedulePage.checkAllDayadnEveryDay();
        if (result1) {

            micResult = true;
            assertTrue(micResult, "Pass:Schedule Configuration pass");
        } else {
            micResult = false;
            assertTrue(micResult, "Failed:Schedule Configuration fail");
        }
            ...
            if (TMSTCPResult == false) {
                micResult =  true;
                assertTrue(micResult,"Pass:Binding the schedule to a Block site/service item can work normally!"  );             
            } else {
                micResult =  false;
                assertTrue(micResult,"Failed:Binding the schedule to a Block site/service item can't work normally!"  );
            }
