package webportal.AP.WiredVLANSupportWAC540564.PRJCBUGEN_T16554;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T16554") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify if user can delete an already existing network using the vlan wizard for WAC540/564") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T16554") // It's a testcase id/link from Jira Test Case.
    ...
    public void step5() {
        APUtils ap = new APUtils(WebportalParam.ap1IPaddress);
        ap.waitConfigReady(vlanId, true);

        String apConf = ap.getConfig(String.format("vlanID %s", vlanId));
        assertTrue(!apConf.contains(vlanId), "vlan id must not be on ap:" + vlanId);
    }
