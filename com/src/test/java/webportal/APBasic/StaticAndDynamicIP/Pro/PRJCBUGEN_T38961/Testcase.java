package webportal.APBasic.StaticAndDynamicIP.Pro.PRJCBUGEN_T38961;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T38961") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("On Insight pro account Change IP to static and then chnage to dynamic and reboot") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN_T38961") // It's a testcase id/link from Jira Test Case.
    ...
    public void step3() {
        ...
        while (i < 10) {
            if (new APUtils(ipMain).getIpStatus1(WebportalParam.ap1Model).contains(ipMain)) {
                result = true;
                break;
            }
            MyCommonAPIs.sleepsync();
        ...
        if (result) {
            assertTrue(new WirelessQuickViewPage().checkApIsOnline(WebportalParam.ap1serialNo)
                    && new WirelessQuickViewPage().checkApIpCorrect(WebportalParam.ap1serialNo, ipMain));
        } else {
            assertTrue(false, "Changed ip address failed.");
        }
    }
