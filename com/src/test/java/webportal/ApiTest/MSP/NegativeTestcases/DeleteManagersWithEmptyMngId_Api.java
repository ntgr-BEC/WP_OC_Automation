...
public class DeleteManagersWithEmptyMngId_Api extends TestCaseBaseApi{
    ...
    @Feature("DeleteManagersWithEmptyMngId_Api") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T001") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("This test deletes managers with empty manager ID to a pro account") // It's a testcase title from Jira Test Case.
    ...
    { 
        ...
        Response getResponse = ApiRequest.sendDeleteRequest(endPointUrl.get("Manager_Sanity"),headers, pathParams, null,400); 
        getResponse.then().body("response.status", equalTo(false))
        .body("response.message", equalTo("Exception in delete manager API"));
