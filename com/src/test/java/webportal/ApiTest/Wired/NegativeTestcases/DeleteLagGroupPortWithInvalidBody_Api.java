...
public class DeleteLagGroupPortWithInvalidBody_Api extends TestCaseBaseApi{
    String networkId;
    String lagGroupId;
    String deviceId;
    ...
    @Feature("DeleteLagGroupPortWithInvalidBody_Api ") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T001") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Deletes the lag group by port With Invalid Json") // It's a testcase title from Jira Test Case.
...
//        Response response1=new Api_GetLagPort().step1();
    ...
    { 
        ...
        //Invalid deviceID
        String InvaliddeviceId=deviceId+"dssfdsf";
        String requestBody="{\"lagConfig\":{\"adminMode\":1,\"lagMembers\":[{\"deviceId\":\""+InvaliddeviceId+"\",\"lagId\":\"0\",\"ports\":[] }],\"name\":\"TestLAGG\",\"type\":1}}";
           
        //TO PERFORM ANY REQUEST    
        Response getResponse = ApiRequest.sendDeleteRequest(endPointUrl.get("LagGroupId_Sanity"), requestBody, headers, pathParams, null, 200); 
        getResponse.then().body("response.status", equalTo(false))
           .body("response.message", equalTo("Invalid deviceId"+InvaliddeviceId));
