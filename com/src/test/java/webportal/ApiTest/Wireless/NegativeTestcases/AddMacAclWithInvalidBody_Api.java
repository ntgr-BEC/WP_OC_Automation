...
public class AddMacAclWithInvalidBody_Api extends TestCaseBaseApi{
    ...
    @Feature("AddMacAclWithInvalidBody_Api") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T001") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("This test Adds MAC ACL config with Invalid JSON") // It's a testcase title from Jira Test Case.
    ...
    { 
        ...
        getResponse2.then().body("response.status", equalTo(false))
                    .body("response.message", equalTo("All mac ACLs failed to add"))
                    .body("response.status", equalTo(false))
                    .body("failureInfo[0].11:33::11:22:34:77.message", equalTo("Failed to validate macAddress and macMaskAddress"));
