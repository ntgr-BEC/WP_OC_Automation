...
public class Api_GetLocSsidDetailsBySsidIdentifer extends TestCaseBaseApi{
    ...
    @Feature("Api_GetLocSsidDetailsBySsidIdentifer") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T004") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Get wireless SSIDs details by identifier [ssid identifier].[Loation ssid in pro account]") // It's a testcase title from Jira Test Case.
    ...
    { 
        ...
        queryParams.put("orgId",WebportalParam.orgId);
        
        Response getResponse = ApiRequest.sendGetRequest(endPointUrl.get("LocSsidDetails_BySsidIdentifier"), headers, pathParams, null); 
        getResponse.then().body("response.status", equalTo(true))
        .body("wirelessNetworkInfo.id", equalTo(id));
