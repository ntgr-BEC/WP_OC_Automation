package webportal.CFD.WAX630E.PRJCBUGEN_T29751;
...
public class Testcase extends TestCaseBase {
    ...
    String width = "Dynamic 20/40/80/160MHz";
    String width1 = "Dynamic 20/40/80MHz";
    @Feature("CFD_630E") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T29751") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify after switching from ICP to basic CP the client is able to connect with SSID.") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T29751") // It's a testcase id/link from Jira Test Case.
    ...
    public void step3() {
        ...
        MyCommonAPIs.sleepi(10);
        new WirelessQuickViewPage(false).DropDown5GhzLow.click(); 
        assertTrue( new WirelessQuickViewPage(false).ChannelWidth5ghzCheck(width1),"channel wideth is not applied for AP2");
        
        new WirelessQuickViewPage().enterDeviceYes(WebportalParam.ap2serialNo);
        new WirelessQuickViewPage(false).RadioAndChannels.click();
        MyCommonAPIs.sleepi(10);
        new WirelessQuickViewPage(false).DropDown5GhzLow.click(); 
        assertTrue( new WirelessQuickViewPage(false).ChannelWidth5ghzCheck(width1),"channel wideth is not applied for AP3");
