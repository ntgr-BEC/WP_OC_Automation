package webportal.CFDPayment.basic.PRJCBUGEN_T21400;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T21400") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Basic Free --> Basic paid -->Premium Annually -->> cancel within 3 days") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T21400") // It's a testcase id/link from Jira Test Case.
    ...
    public void step1() {
        ...
        accountInfo.put("Last Name", "T21400");
    ...
    @Step("Test Step 2: Buy device credits;")
    public void step2() {
        if (new HamburgerMenuPage(true).checkSubscriptionScreenForBasic()) {
            ...
            paymentInfo.put("Last Name", "T21400");
//            paymentInfo.put("Email", mailname + "@mailinator.com");
            ...
            assertTrue(new HamburgerMenuPage().checkDevicesBilling(paymentInfo.get("Number of Device Credits"), "basic", "US"),
                    "Amount is incorrect.");
        } else {
    ...
    @Step("Test Step 3: Buy yearly premium subscription, then cancel subscription;")
    public void step3() {
        new HamburgerMenuPage(false).changePlanToPremium(paymentInfo);

        if (new HamburgerMenuPage(false).checkSubscriptionScreenForPremium()) {
            new HamburgerMenuPage(false).cancelSubscription();

            assertTrue(!new HamburgerMenuPage(false).currentSubscriptionNew.exists(), "Premium subscription type incorrect.");
        } else {
            assertTrue(false, "Premium subscription type incorrect.");
        }
    }
