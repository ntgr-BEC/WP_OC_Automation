package webportal.CFTopupSubscription.PRJCBUGEN_T27202;
...
public class Testcase extends TestCaseBase {
    ...
    @Feature("CFTopupSubscription") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T27202") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify the content filter service subscription functionality from insight service market place") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T27202") // It's a testcase id/link from Jira Test Case.
    ...
    public void step3() {
        ...
        Map<String, String> paymentInfo = new HashMap<String, String>();
        paymentInfo.put("Subscription Type", "Top-up Inspection Limit Pack");
        ...
        new InsightServicesPage(false).buyCFTopUp(paymentInfo);
        
        assertTrue(
                new ContentFilteringPage().checkBuyCFTopUpResult(paymentInfo.get("YearNum"), paymentInfo.get("Quantity")),
                "Vpn services page display incorrect.");
        assertTrue(new ContentFilteringPage().checkCFTopUpSubscription(paymentInfo.get("Quantity"), paymentInfo.get("Quantity")),"CF page is not right");
