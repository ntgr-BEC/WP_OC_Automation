package webportal.ConfigBackupAndRestore.Premium.NotImplemented.PRJCBUGEN_T9457;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T9457") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify the Facebook Wifi for wireless device at location level") // It's a test case title from
    // Jira Test Case.
    @TmsLink("PRJCBUGEN-T9457") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 3: Add ssid and enable facebook wifi, then create second backup;")
    public void step3() {
        handle.gotoLoction();

        ssidInfo.put("SSID", "apwp9457");
        ssidInfo.put("Security", "WPA2 Personal");
        ssidInfo.put("Password", "12345678");
        new WirelessQuickViewPage().addSsid(ssidInfo);
        new WirelessQuickViewPage().enableFacebookWifi(ssidInfo.get("SSID"));

        BackupInfoTwo.put("Name", "test9457two");
    ...
    public void step4() {
        new AccountPage().restoreBackup(WebportalParam.location1, BackupInfo.get("Name"));
        handle.gotoLoction();
        if (!new WirelessQuickViewPage().checkFacebookWifiIsEnable(ssidInfo.get("SSID"))) {
            new AccountPage().restoreBackup(WebportalParam.location1, BackupInfoTwo.get("Name"));
            handle.gotoLoction();
            assertTrue(new WirelessQuickViewPage().checkFacebookWifiIsEnable(ssidInfo.get("SSID")), "Restore backup failed.");
        } else {
