package webportal.CreateLocation.WP.PRJCBUGEN_T13862;
...
public class Testcase extends TestCaseBase {
    final static Logger logger = Logger.getLogger("PRJCBUGEN_T13862");  
    ...
    @Story("PRJCBUGEN_T13862") 
    @Description("Verify that the user is able to create multiple locations in a pro account (Manager)") 
    @TmsLink("PRJCBUGEN_T13862") 
   ...
   @Step("Test Step 1: Log in to a pro account admin for creating org ")
    ...
    locationInfo.put("Device Admin Password", WebportalParam.adminPassword);
...
public void step4() {       
    try {        
        ...
        for (SelenideElement locelem : esc) {
            System.out.println("locelem.getText() "+locelem.getText());
        ...
        if(located ==2 && result) {
            assertTrue(result, "Verified  that the user is able to create multiple locations in a pro account R/W Manager");
            logger.info("Verified  that the user is able to create multiple locations in a pro account R/W Manager");
        }
        else {
            assertTrue(false, "Verified  that the user is able to create multiple locations in a pro account R/W Manager");                   
        }
