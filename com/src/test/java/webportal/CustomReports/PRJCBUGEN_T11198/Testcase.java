package webportal.CustomReports.PRJCBUGEN_T11198;
...
public class Testcase extends TestCaseBase {
    ...
    String      reportName = "PRJCBUGEN_T11198";
    MailHandler mh         = new MailHandler();

    @Feature("Custom Reports") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T11198") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify if the FW status widgets correctly shows the Info across all the devices") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T11198") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: create reports of type for fw widget")
    public void step2() {
        crPage.dataReportName = reportName;
        crPage.dataMailAddress = mailRecv;
        crPage.gotoPage();
        crPage.setReport(reportName, 5);
    ...
    public void step3() {
        ...
        for (String s : MyCommonAPIs.getTexts(crPage.txtReviewHeader.getSearchCriteria())) {
            if (s.contains("irmware")) {// Firmware Status
                micResult = true;
            }
        }
        assertTrue(micResult, "check must have Firmware Status changes");
