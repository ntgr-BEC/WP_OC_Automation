package webportal.CustomReports.PRJCBUGEN_T11203;
...
public class Testcase extends TestCaseBase {
    ...
    String      reportName = "PRJCBUGEN_T11203";
    MailHandler mh         = new MailHandler();
    
    @Feature("Custom Reports") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T11203") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify if the troubleshoot widget shows top ports with consumption accurately.") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T11203") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: create troubleshoot reports for Top 5 Ports by Utilization widgets")
    public void step2() {
        crPage.dataReportName = reportName;
        crPage.dataMailAddress = mailRecv;
        crPage.dataReportType = 1;
        crPage.gotoPage();
        crPage.setReport(reportName, 7);
    ...
    public void step3() {
        ...
        for (String s : MyCommonAPIs.getTexts(crPage.txtReviewHeader.getSearchCriteria())) {
            if (s.contains("Utilization")) {
                micResult = true;
            }
        }
        assertTrue(micResult, "check must have Top 5 Ports by Utilization ");
