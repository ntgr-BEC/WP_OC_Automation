package webportal.DeviceGroup.FeaturesInheritedfromLocation.PRJCBUGEN_T32437;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T32437") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify, syslog config is created for backup > on-board device to CGs then restored backup : verify syslog config's is restored on location devices") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T32437") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 3: Go to Syslog and enable")
    public void step3() {
        new DeviceGroupPage().GoToSysLog(WebportalParam.location1);
    ...
    public void step4() {
        ...
        if(result.contains("logSettings:syslogStatus 0") && result.contains("logSettings:syslogSrvIp 1.1.1.1") && result.contains("logSettings:syslogSrvPort 514")) {
            status = true;
        }
