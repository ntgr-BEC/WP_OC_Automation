package webportal.DirectPurchase.NewPremiumUser.PRJCBUGEN_T18450;
...
public class Testcase extends TestCaseBase {
    ...
    String              newPassword = "Netgear1@";

    @Feature("DirectPurchase.NewPremiumUser") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T18450") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify if user able to to login with changed password") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T18450") // It's a testcase id/link from Jira Test Case.
...
//        new HamburgerMenuPage(false).clickLoginAndSubscribe("pro");
        ...
        accountInfo.put("Last Name", "T18450");
    ...
    @Step("Test Step 2: Buy pro subscription and change password;")
    public void step2() {
        Map<String, String> paymentInfo = new HashMap<String, String>();
        paymentInfo.put("First Name", mailname);
        paymentInfo.put("Last Name", "T18450");
        ...
        if (new HamburgerMenuPage(true).checkProAccountSubscriptionInfo()) {
            ...
            new HamburgerMenuPage().changePassowrd(password, newPassword);
        } else {
            assertTrue(false, "Subscription incorrect.");
        }
    }

    @Step("Test Step 3: Login with new password;")
    public void step3() {
        UserManage userManage = new UserManage();
        userManage.logout();

        new WebportalLoginPage().loginByUserPassword(mailname + "@mailinator.com", newPassword);
        assertTrue(new HamburgerMenuPage(false).checkLoginSuccessful(), "Login failed.");
    }
