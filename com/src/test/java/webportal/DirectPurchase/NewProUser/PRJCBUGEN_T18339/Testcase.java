package webportal.DirectPurchase.NewProUser.PRJCBUGEN_T18339;
...
public class Testcase extends TestCaseBase {
    ...
    String              organizationName = "test18339";

    @Feature("DirectPurchase.NewProUser") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T18339") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify user able to Deallocate device credit to location in Organization") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T18339") // It's a testcase id/link from Jira Test Case.
    ...
    public void step1() {
        ...
        accountInfo.put("Last Name", "T18339");
    ...
    @Step("Test Step 2: Buy pro subscription and check deallocation;")
    public void step2() {
        Map<String, String> paymentInfo = new HashMap<String, String>();
        paymentInfo.put("First Name", mailname);
        paymentInfo.put("Last Name", "T18339");
        ...
        if (creditsInfo.get("Total Devices Credits").equals("1") && creditsInfo.get("Used Devices Credits").equals("0")
                ...
                && creditsInfo.get("Unused ICP Credits").equals("0")) {
            new HamburgerMenuPage().deallocateCredit(organizationName, "1", "0");

            creditsInfo = new HamburgerMenuPage().getCreditAllocationStatus(organizationName);
            assertTrue(creditsInfo.get("Total Devices Credits").equals("0") && creditsInfo.get("Used Devices Credits").equals("0")
                    && creditsInfo.get("Unused Devices Credits").equals("0") && creditsInfo.get("Total VPN Credits").equals("0")
                    && creditsInfo.get("Used VPN Credits").equals("0") && creditsInfo.get("Unused VPN Credits").equals("0")
                    && creditsInfo.get("Total ICP Credits").equals("0") && creditsInfo.get("Used ICP Credits").equals("0")
                    && creditsInfo.get("Unused ICP Credits").equals("0"), "Allocate credits error.");
        } else {
            assertTrue(false, "Allocate credits error.");
        }
    }
