package webportal.FlexiProDeallocation.PRJCBUGEN_T17023;
...
public class Testcase extends TestCaseBase {
    ...
    String              organizationName    = "test17023";
    ...
    @Story("PRJCBUGEN_T17023") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify that admin able to allocate the deallocated credits to another organisation in the same account") // It's a test case title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T17023") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        System.out.println(creditsInfo);
        assertTrue(creditsInfo.get("Deallocate DevNum").equals(String.valueOf(devNumNew + devNum))
                && creditsInfo.get("Deallocate IcpNum").equals(String.valueOf(icpNumNew + icpNum)), "Allocate credits error.");
    }

    @Step("Test Step 3: Deallocate credits, then check devices credits page;")
    public void step3() {
        new HamburgerMenuPage().deallocateCredit(organizationName, String.valueOf(devNum), String.valueOf(icpNum));
        new HamburgerMenuPage().configCreditAllocation(organizationInfoNew.get("Name"), devNum, 0, icpNum);
        ...
        assertTrue(creditsInfo.get("Used Devices Credits").equals("0") && creditsInfo.get("Unused Devices Credits").equals("0")
                ...
                && creditsInfoNew.get("Used Devices Credits").equals("0")
                && creditsInfoNew.get("Unused Devices Credits").equals(String.valueOf(devNum + devNumNew))
                && creditsInfoNew.get("Used ICP Credits").equals("0")
                && creditsInfoNew.get("Unused ICP Credits").equals(String.valueOf(icpNum + icpNumNew)), "Allocate credits error.");
    }
