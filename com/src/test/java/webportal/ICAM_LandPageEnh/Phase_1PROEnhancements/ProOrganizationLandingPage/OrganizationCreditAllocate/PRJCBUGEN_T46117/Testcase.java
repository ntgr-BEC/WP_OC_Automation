package webportal.ICAM_LandPageEnh.Phase_1PROEnhancements.ProOrganizationLandingPage.OrganizationCreditAllocate.PRJCBUGEN_T46117;
...
public class Testcase extends TestCaseBase {
    ...
    @Feature("OrganizationCreditAllocate") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T46117") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description(" Verify Used and Unused Credits are Displayed Properly") // It's                                                                                                                                                     // Jira
    // Test
    // Case.
    @TmsLink("PRJCBUGEN_T46117") // It's a testcase id/link from Jira Test Case.
    ...
    public void step1() {
      ...
      @Step("Test Step 2: Create new organization and allocate device credits")
      public void step2() {
          ...
          assertTrue(OrganizationPage.verifyAllocatedDeviceCredits("Netgear1"),"device Credits not allocated successfully");
      ...
      @Step("Test Step 4: Verify used and unsed creds")
      public void step4() {
          
          new OrganizationPage(false).gotoPage();
          MyCommonAPIs.sleepi(5);
          assertTrue(new OrganizationPage(false).verifyUsedUnsedCreditsAfterOnboardingdevices("Netgear1"),"Used Unused creds not verified");
          new OrganizationPage(false).openOrg("Netgear1");
          new MyCommonAPIs().getLocationCurrentLocation(WebportalParam.location1);
          new WirelessQuickViewPage().deleteDeviceYes(WebportalParam.ap5serialNo);
          new WirelessQuickViewPage().deleteDeviceYes(WebportalParam.sw2serialNo);
          new WirelessQuickViewPage().deleteDeviceYes(WebportalParam.ob2serialNo);
          new OrganizationPage(false).gotoPage();
          MyCommonAPIs.sleepi(5);
          assertTrue(new OrganizationPage(false).verifyUsedUnsedCreditsAfterOnboardingNodevices("Netgear1"),"Used Unused creds not verified");
      }
