package webportal.ICAM_LandPageEnh.Phase_1PROEnhancements.ProOrganizationLandingPage.RouteRedirecting.PRJCBUGEN_T46143;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T46143") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify Table Behavior After Clearing Filters.") // It's // Jira
    // Test
    // Case.
    @TmsLink("PRJCBUGEN_T46143") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: Verify On Organization dashboard online devices ")
    public void step2() {
    ...
    @Step("Test Step 3: Verify On Organization dashboard offline filter, then clear filter and verify all devices are showing or not")
    public void step3() {

        new OrganizationPage(false).gotoPage();
        MyCommonAPIs.sleepi(5);
        assertTrue(new OrganizationPage(false).isFilterActive("Offline Devices", "Offline"), "After Clicking all offline devices there offline devices filter is not active");
        new OrganizationPage(false).gotoPage();
        MyCommonAPIs.sleepi(5);
        assertTrue(new OrganizationPage(false).verifyOrgDashboardAndOrgSettingsDevicesTableWhenclickonHeaderTabs(WebportalParam.ap1serialNo, WebportalParam.sw1serialNo,
                WebportalParam.ob1serialNo, "Offline Devices", WebportalParam.ap5serialNo, WebportalParam.sw2serialNo,
                WebportalParam.ob2serialNo), "After clearing filter  all devices are not showing");
