package webportal.ICP.manager.PRJCBUGEN_T16654;
...
public class Testcase extends TestCaseBase {
    ...
    @Feature("ICP.manager") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T16654") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("verify instant captive portal funcationally when user select tier type as \"voucher \"") // It's a test case title from
                                                                                                           // Jira Test Case.
    @TmsLink("PRJCBUGEN-T16654") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        new WirelessQuickViewPage().deleteSsidYes("apwp16654");
        ...
        accessCode = new WirelessQuickViewPage().getAccessCodeOnManager(ssidInfo.get("SSID"));

        int sum = 0;
        while (true) {
            MyCommonAPIs.sleepi(10);
            if (new Javasocket()
                    .sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "WAFfindSSID apwp16654")
                    .indexOf("true") != -1) {
                break;
            } else if (sum > 30) {
        ...
        if (!new Javasocket()
                .sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "WAFconnect apwp16654 12345678 WPA2PSK aes")
                .equals("true")) {
            result1 = false;
