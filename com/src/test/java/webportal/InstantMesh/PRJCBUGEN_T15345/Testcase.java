package webportal.InstantMesh.PRJCBUGEN_T15345;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T15345") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify the Mode Change from Auto (Root)to Root Only and Reboot the AP") // It's a test case title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T15345") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        if (new DevicesDashPage().waitDevicesReConnected(sNo)) {
            new DevicesDashPage().enterDevice(WebportalParam.ap3serialNo);
            new DevicesApMeshTopologyPage().clickMeshRootAp();
            assertTrue(new DevicesApMeshTopologyPage().checkMeshStatusAndAdvanceSettings(""), "Mesh ap advance settings is incorrect.");
        } else {
            assertTrue(false, "Root ap cannot connect.");
        }
    }
