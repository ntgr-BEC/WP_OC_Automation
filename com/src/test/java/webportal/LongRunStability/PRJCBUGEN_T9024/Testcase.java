package webportal.LongRunStability.PRJCBUGEN_T9024;
...
public class Testcase extends TestCaseBase {
    final String    tcName       = "[T9024] Check AP device info on Cloud";
    String          stepInfo     = "";
    String          expectResult = "";
    String          actualResult = "";
    ...
    @Story("PRJCBUGEN_T9024") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description(tcName) // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T9024") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: Go to network1, check AP Status is connected;")
    public void step2() {
        try {
            stepInfo = "Check AP Status is connected in 1st network";
            handle.gotoLoction(WebportalParam.location1);
            ddp.gotoPage();
            String apNo = ddp.getAPDevice();
            ...
            stepInfo = "Check AP uptime in 1st network";
            expectResult = "Not NA";
    ...
    @Step("Test Step 3: Go to network2, check AP Status is connected;")
    public void step3() {
        stepInfo = "Check AP Status is connected in 2nd network";
        ...
        stepInfo = "Check AP uptime in 2nd network";
        expectResult = "Not NA";
        actualResult = ddp.getDeviceUptime(apNo, false);
        if (!actualResult.equals("NA")) {
            dbHandle.writeResultPass(tcName, stepInfo, expectResult, actualResult);
