package webportal.LongRunStability.PRJCBUGEN_T9027;
...
public class Testcase extends TestCaseBase {
    final String    tcName       = "[T9027] Verify the configuration deploy NAS from Insight";
    String          stepInfo     = "";
    String          expectResult = "";
    String          actualResult = "";
    SQLiteStability dbHandle     = null;
    String          devNo        = null;
    String          devMode      = "LongRun-NAS-1";
    String          devNewName   = "LR-NAS-";
    String          devIp        = null;

    @Feature("LongRunStability") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T9027") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description(tcName) // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T9027") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: Go to network1, edit NAS name")
    public void step2() {
        handle.gotoLoction(WebportalParam.location1);
        ddp.gotoPage();
        devNo = ddp.getNASDevice();
        devMode = ddp.getDeviceModel(devNo);
        devMode = devMode.replace("ReadyNAS ", "").trim();
        devIp = ddp.getDeviceIP(devNo);

        devNewName = String.format("%s-%s", new SimpleDateFormat("MMddHH").format(new Date().getTime()), devMode);
        expectResult = String.format("Device name should be changed to: '%s'", devNewName);
        stepInfo = "Verify NAS device name can be edit on webportal";
        ...
        stepInfo = "Check NAS new name is pushed to device: " + devIp;
        MyCommonAPIs.sleepsync();
        devicename = new PlinkCmd(devIp).getNASName();
