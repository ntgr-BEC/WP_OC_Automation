package webportal.MarketPlace.Premium.PRJCBUGEN_T10284;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T10284") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify that user can buy multiple subscriptions of different durations") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T10284") // It's a testcase id/link from Jira Test Case.
    ...
    public void step1() {
        ...
        accountInfo.put("Last Name", "T10284");
...
//        new HamburgerMenuPage(false).clickAddInsightIncludedDevices();
        ...
        paymentInfo.put("First Name", mailname);
        paymentInfo.put("Last Name", "T10284");
        ...
        boolean result = true;
        for (int year = 1; year < 6; year += 2) {
            if (!new HamburgerMenuPage().checkBuySomeVpnResult(String.valueOf(year), paymentInfo.get("BuyNum"))) {
                result = false;
                break;
            }
        }

        assertTrue(result, "Order history display incorrect.");
    }
