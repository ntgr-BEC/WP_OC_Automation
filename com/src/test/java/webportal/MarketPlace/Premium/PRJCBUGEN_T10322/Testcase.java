package webportal.MarketPlace.Premium.PRJCBUGEN_T10322;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T10322") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify that is user able to delete VPN group if subscription has expired.") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T10322") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        if (new HamburgerMenuPage(false).checkSubscriptionScreen(paymentInfo.get("Number of Device Credits"))) {
            ...
            vpnPaymentInfo.put("Last Name", "T10322");
    ...
    @Step("Test Step 3: Add three vpn groups and cancel vpn services, check delete one vpn group;")
    public void step3() {
        ...
        if (brrp.checkExpiredVpnGroupStatus("vpn0") && brrp.checkExpiredVpnGroupStatus("vpn1")) {
            brrp.deleteGroupNew("vpn0");

            brrp.gotoPage();
            assertTrue(!brrp.checkVpnGroupIsExist("vpn0"), "<2>Delete vpn group unsuccess.");
        } else {
