package webportal.MarketPlace.Premium.PRJCBUGEN_T10323;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T10323") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify that, are user configs being stored after expiration") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T10323") // It's a testcase id/link from Jira Test Case.
    ...
    public void tearDown() {
        new WirelessQuickViewPage().deleteDeviceYes(WebportalParam.br2serialNo);
    ...
    public void step1() {
        ...
        accountInfo.put("Last Name", "T10327");
    ...
    public void step2() {
        ...
        if (new HamburgerMenuPage(false).checkSubscriptionScreen(paymentInfo.get("Number of Device Credits"))) {
            ...
            vpnPaymentInfo.put("Last Name", "T10323");
    ...
    @Step("Test Step 3: Add three vpn groups and cancel vpn services, check group limit;")
    public void step3() {
        ...
        brrp.gotoPage();
        if (brrp.checkExpiredVpnGroupStatus("vpn0") && brrp.checkExpiredVpnGroupStatus("vpn1")) {
            vpnPaymentInfo.put("BuyNum", "1");
            new InsightServicesPage().buyVpnProducts(vpnPaymentInfo);

            brrp.gotoPage();
            if (brrp.checkExpiredVpnGroupStatus("vpn1")) {
                brrp.deleteGroupNew("vpn1");

                brrp.gotoPage();
                assertTrue(!brrp.checkExpiredVpnGroupStatus("vpn0"), "<3>Vpn group locked.");
            } else {
                assertTrue(false, "<2>Vpn group not locked.");
            }
        } else {
            assertTrue(false, "<1>Vpn group not locked.");
        }
    }
