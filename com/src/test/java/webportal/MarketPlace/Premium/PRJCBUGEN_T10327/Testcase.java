package webportal.MarketPlace.Premium.PRJCBUGEN_T10327;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T10327") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify that VPN functionality gets unlocked after renewing VPN subscription") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T10327") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        if (new HamburgerMenuPage(false).checkSubscriptionScreen(paymentInfo.get("Number of Device Credits"))) {
            ...
            vpnPaymentInfo.put("Last Name", "T10327");
    ...
    public void step3() {
        ...
        brrp.gotoPage();
        if (brrp.checkVpnGroupNumLimit("buy now")) {
            vpnPaymentInfo.put("BuyNum", "3");

            new InsightServicesPage().buyVpnProducts(vpnPaymentInfo);
            brrp.gotoPage();
            brrp.createVPNGroup(vpnGroupName + String.valueOf(i));
            assertTrue(brrp.checkVpnGroupIsExist("vpn3"), "Vpn group credits is incorrect.");
        } else {
