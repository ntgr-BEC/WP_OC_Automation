package webportal.MultiAdmin.PRJCBUGEN_T30366;
...
public class Testcase extends TestCaseBase {
    ...
    String Name     = "PRJCBUGEN_T30366";

    @Feature("MultiAdmin") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T30366") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify the behaviour of Secondary admin when access is revoked") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T30366") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 3: check secondary account ")
    public void step3() {
    
        String AccountType = new HamburgerMenuPage(false).checkUserLoggedInn();     
        assertTrue(AccountType.equals("Pro Secondary Admin"), "Account type is wrong");
    ...
    @Step("Test Step 5: login to secondary admin secondary admin ")
    public void step5() {
    
        UserManage userManage = new UserManage();
        userManage.logout();   
        WebportalLoginPage webportalLoginPage = new WebportalLoginPage(true);
        webportalLoginPage.loginByUserPassword(mailname ,WebportalParam.adminPassword);
        new HamburgerMenuPage();
    }
  
    @Step("Test Step 6: login to secondary admin secondary admin ")
    public void step6() {
    
        String AccountType = new HamburgerMenuPage(false).checkUserLoggedInn();     
        assertTrue(AccountType.equals("Premium"), "Account type is wrong");
