package webportal.NatMode.user.Part2.prouser.PRJCBUGEN_T33934;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T33934") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify, RATELIMIT ON SSID WITH NAT MODE") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN_T33934") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: Add WIFI ssid in natmode and configure rate limit;")
    public void step2() {
        ...
        while (true) {
            ...
            if (new Javasocket().sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "WAFfindSSID apwp14008")
                    .indexOf("true") != -1) {
                break;
            } else if (sum > 30) {
    ...
    public void step3() {
        ...
        if (str.indexOf("ERROR:") == -1 && !str.contains("finalresult: 0")) {
            ...
            if (matcher.find()) {
                ...
                assertTrue(Float.parseFloat(downloadSpeed) < 27 && Float.parseFloat(downloadSpeed) > 0 && Float.parseFloat(uploadSpeed) < 47
