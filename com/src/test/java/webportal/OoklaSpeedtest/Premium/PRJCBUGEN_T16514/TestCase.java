package webportal.OoklaSpeedtest.Premium.PRJCBUGEN_T16514;
...
public class TestCase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T16514") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify if the unit for Download and Upload speeds are proper") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN_T16514") // It's a testcase id/link from Jira Test Case.
...
//        } 
    ...
    public void step3() {
        ...
        if (!UploadInfo.equals("-") && !DownloadInfo.equals("-")) {
            Result = true;
        }
        if (Result == true) {
            micResult = true;
            assertTrue(micResult,"Pass:Get the network upload and download speed with appropriate unit successfully !");  
        }else {
            micResult = false;
            assertTrue(micResult,"Failed:Get the network upload and download speed with appropriate unit unsuccessfully!"); 
        } 
