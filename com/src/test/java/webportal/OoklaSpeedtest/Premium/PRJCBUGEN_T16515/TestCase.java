package webportal.OoklaSpeedtest.Premium.PRJCBUGEN_T16515;
...
public class TestCase extends TestCaseBase{
    ...
    @Story("PRJCBUGEN_T16515") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify whether the units for latency results are proper") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN_T16515") // It's a testcase id/link from Jira Test Case.
...
//        } 
    ...
    @Step("Test Step 3: Now once the device comes online, go to the device dashboard and go to Troubleshoot -> Ookla Speedtest page to get latency info")
    public void step3() {
        boolean Result = false;
        DevicesDashPage DevicesDashPage= new DevicesDashPage();
        ...
        if (Result == true) {
            micResult = true;
            assertTrue(micResult,"Pass:Get the network latency with appropriate unit successfully !");  
        }else {
            micResult = false;
            assertTrue(micResult,"Failed:Get the network latency with appropriate unit unsuccessfully!"); 
        } 
