package webportal.Orbi.ORBIPRO_SSID_Management.WIFI6SSIDVLAN.PRJCBUGEN_T23387;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T23387") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify that creating modifying a VLAN on a particular SSID should not affect the clients connected on any SSID having the same VLAN configured.") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T23387") // It's a testcase id/link from Jira Test Case.
    ...
    public void step3() {
        wifiNetwork.ssidData.ssid = "T23387-" + RandomStringUtils.randomNumeric(5);
        wifiNetwork.ssidData.enaSsidVlan = true;
        wifiNetwork.ssidData.ssidVlan = "Employee";
        wifiNetwork.editSsidSetting(2);

        handle.waitRestReady(BRUtils.api_device_iot_wlan_info, wifiNetwork.ssidData.ssid, false, 3);
        assertTrue(new BRUtils().getFieldsOrbi("vlanProfile").contains("20"), "check vlan name can be changed to Employee");
    }
}
