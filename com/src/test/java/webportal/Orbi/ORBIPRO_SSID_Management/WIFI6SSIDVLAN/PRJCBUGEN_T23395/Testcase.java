package webportal.Orbi.ORBIPRO_SSID_Management.WIFI6SSIDVLAN.PRJCBUGEN_T23395;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T23395") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify the MVLAN functionality when Pofile VLAN is configured with same and different VLAN") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T23395") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ddp.openOBDevice();
        orbiNetwork.gotoPage();
        orbiNetwork.networkData.vlanName = "test";
        orbiNetwork.networkData.vlanId = "30";
        try {
            orbiNetwork.createNetwork();
        } catch (Throwable e) {
            assertTrue(orbiNetwork.imgWarning.exists(), "check failed for vlan existed");
        }
    }
    
    @Step("Test Step 3: Create a new VLAN profile")
    public void step3() {
        orbiNetwork.gotoPage();
        orbiNetwork.networkData.vlanName = vlanName;
        orbiNetwork.networkData.vlanId = vlanId;
        orbiNetwork.createNetwork();
        needClean = true;
    }
