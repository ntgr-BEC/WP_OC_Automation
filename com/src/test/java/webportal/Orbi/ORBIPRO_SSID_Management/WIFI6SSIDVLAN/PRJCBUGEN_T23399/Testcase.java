package webportal.Orbi.ORBIPRO_SSID_Management.WIFI6SSIDVLAN.PRJCBUGEN_T23399;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T23399") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify the VLAN associated to the DHCP server") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T23399") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: Create a new VLAN profile in Network Setup")
    public void step2() {
        ddp.openOBDevice();
        orbiNetwork.gotoPage();      
    ...
    @Step("Test Step 3: choose newly created VLAN profile in drop down menu")
    public void step3() {
        orbiDhcp.gotoPage();
        orbiDhcp.dumpInfo();
        handle.waitRestReady(BRUtils.api_lan_subnet, vlanId, false, 3);
        assertTrue(new BRUtils().Dump().contains(vlanId), "check dhcp vlan id is changed");
    }
}
