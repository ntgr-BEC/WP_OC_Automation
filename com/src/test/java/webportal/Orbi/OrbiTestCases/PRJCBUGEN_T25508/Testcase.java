package webportal.Orbi.OrbiTestCases.PRJCBUGEN_T25508;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T25508") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify device rename while device onboarding") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T25508") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {        
        ...
        devInfo.put("Device Name", WebportalParam.ob2deveiceName);

        new DevicesDashPage(false).addNewDevice(devInfo);      
    }

    @Step("Test Step 3: check device name and device status")
    public void step3() {
        ddp.gotoPage();
        //assertTrue(ddp.getDeviceName(WebportalParam.ob2serialNo).equals(WebportalParam.ob2serialNo), "device name is not sn");
        assertTrue(new DevicesDashPage().waitDevicesReConnected(WebportalParam.ob2serialNo), "device cannot online.");
        ddp.gotoPage();
        assertTrue(ddp.getDeviceName(WebportalParam.ob2serialNo).equals(WebportalParam.ob2deveiceName), "device name is not updated");
    }
