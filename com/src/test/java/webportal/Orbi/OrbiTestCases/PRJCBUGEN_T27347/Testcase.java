package webportal.Orbi.OrbiTestCases.PRJCBUGEN_T27347;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T27347") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify, on-boarding day-0 SXK30 EU-SKU and changing any parameter of default SSID will not disconnect SXK30 from insight ") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T27347") // It's a testcase id/link from Jira Test Case.
    ...
    public final static Map<String, String> device_setup_router_Info_ssidpw = new HashMap<String, String>() {
        /**
         * 
         */
...

        {
            ...
            put("WifiNetworkPwd", WebportalParam.ob2WifiNetworkPwd1 + "new");
    ...
    @Step("Test Step 6: Setup orbi wizard router mode via webportal / Check device is not disconnected with cloud")
    public void step6() {
       
        new DevicesDashPage(false).gotoPage();
        new DevicesDashPage().enterDevice(WebportalParam.ob2serialNo);
        DevicesOrbiSetupWizardPage setupWizardPage = new DevicesOrbiSetupWizardPage();
        setupWizardPage.SetupOrbiDayODevice(device_setup_router_Info_ssidpw);

        MyCommonAPIs.sleepsyncorbi();
        assertTrue(new DevicesDashPage().waitDevicesReConnected(WebportalParam.ob2serialNo), "Device disconnect with cloud");
    }
