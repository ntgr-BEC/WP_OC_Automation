package webportal.Orbi.OrbiTestCases.PRJCBUGEN_T27395;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T27395") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("verify, on day-0 config > last screen on pending  popup  with message \"initial configure will take some time to apply\"") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T27395") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 6: Setup orbi wizard router mode via webportal / Check pending's last popup msg")
    public void step6() {
       
        new DevicesDashPage(false).gotoPage();
        new DevicesDashPage().enterDevice(WebportalParam.ob2serialNo);
        DevicesOrbiSetupWizardPage setupWizardPage = new DevicesOrbiSetupWizardPage();
        boolean result = setupWizardPage.SetupOrbiDayODeviceCheckLastMsg(device_setup_router_Info_ssidpw);

        MyCommonAPIs.sleepsync();
        assertTrue(result, "Last popup msg not correct");
    }
