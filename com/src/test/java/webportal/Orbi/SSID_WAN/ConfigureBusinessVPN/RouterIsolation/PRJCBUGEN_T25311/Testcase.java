package webportal.Orbi.SSID_WAN.ConfigureBusinessVPN.RouterIsolation.PRJCBUGEN_T25311;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T25311") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Check whether we are able to ping Central router from remote router where Router isolation is enabled") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T25311") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        page.clickNext();
        page.setPage3WirelessSettings(true, "PRJCBUGEN-T25311", 1, "12345678");
        page.clickNext();
        ...
        page.gotoPage();
        checkpoint = page.GroupExist("TestGroup", "PRJCBUGEN-T25311", WebportalParam.ob1deveiceName, "Healthy");
        if(!checkpoint) {
            util.MyCommonAPIs.sleepsyncorbi();
        }
    }
    ...
    @Step("Test Step 3: Check client connect to remote router can ping central router")
    public void step3() {
        ...
        for(int i=0;i<3;i++) {
            ...
            // connect to bssid
            checkpoint = true;
            cmd = String.format("WAFconnectBSSID PRJCBUGEN-T25311 12345678 WPA2PSK aes %s", WebportalParam.ob2SSIDWANbssid);
            if (!new Javasocket()
        ...
        assertTrue(checkpoint, "checkpoint3 : client connect to remote router can ping central router");
    }
