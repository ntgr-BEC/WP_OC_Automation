package webportal.Orbi.SSID_WAN.ConfigureBusinessVPN.RouterIsolation.PRJCBUGEN_T25313;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T25313") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Check whether after reboot the router isolation configuration is still intact and works fine") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T25313") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        page.clickNext();
        page.setPage3WirelessSettings(true, "PRJCBUGEN-T25313", 1, "12345678");
        page.clickNext();
        ...
        page.gotoPage();
        checkpoint = page.GroupExist("TestGroup", "PRJCBUGEN-T25313", WebportalParam.ob1deveiceName, "Healthy");
        if(!checkpoint) {
            util.MyCommonAPIs.sleepsyncorbi();
        }
    }
    
    @Step("Test Step 3: Reboot device.")
    public void step3() {
    ...
    @Step("Test Step 4: Check client connect to employee home router cannot ping branch office router")
    public void step4() {
        ...
        for(int i=0;i<3;i++) {
            ...
            // connect to bssid
            checkpoint = true;
            cmd = String.format("WAFconnectBSSID PRJCBUGEN-T25313 12345678 WPA2PSK aes %s", WebportalParam.ob2SSIDWANbssid);
            if (!new Javasocket()
        ...
        assertTrue(checkpoint, "checkpoint 2 : after reboot client connect to employee home router cannot ping branch office router");
    }
