package webportal.Orbi.SSID_WAN.ConfigureBusinessVPN.WirelessSettings.PRJCBUGEN_T25303;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T25303") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to Verify the VPN SSID working with the password or not") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T25303") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        page.gotoPage();
        if(!page.GroupExist("TestGroup", "PRJCBUGEN-T25303", WebportalParam.ob1deveiceName, "Healthy")) {
            util.MyCommonAPIs.sleepsyncorbi();
        }
    }
    ...
    public void step3() {
        ...
        for(int i=0;i<3;i++) {
            ...
            // connect to bssid
            checkpoint = true;
            cmd = String.format("WAFconnectBSSID PRJCBUGEN-T25303 12345678 WPA2PSK aes %s", WebportalParam.ob1SSIDWANbssid);
            if (!new Javasocket()
            ...
            checkpoint = true;
            cmd = String.format("WAFconnectBSSID PRJCBUGEN-T25303 12345678 WPA2PSK aes %s", WebportalParam.ob2SSIDWANbssid);
            if (!new Javasocket()
