package webportal.Orbi.SSID_WAN.ConfigureBusinessVPN.WirelessSettings.PRJCBUGEN_T25342;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T25342") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify whether clients which connected in VPN SSID via satellite is also listed in the client connected table and in Business VPN client graph") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T25342") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        page.clickNext();
        page.setPage3WirelessSettings(true, "PRJCBUGEN-T25342", 1, "12345678");
        page.clickNext();
        ...
        util.MyCommonAPIs.sleepsyncorbi();
        page.gotoPage();
        if(!page.GroupExist("TestGroup", "PRJCBUGEN-T25342", WebportalParam.ob1deveiceName, "Healthy")) {
            util.MyCommonAPIs.sleepsyncorbi();
        }
    }
    ...
    public void step3() {
        ...
        for(int i=0;i<3;i++) {
            ...
            // connect to bssid
            checkpoint = true;
            cmd = String.format("WAFconnectBSSID PRJCBUGEN-T25342 12345678 WPA2PSK aes %s", satellite_bssid);
            if (!new Javasocket()
        ...
        util.MyCommonAPIs.sleepi(120); // wait the client update to Insight
    }
    
    @Step("Test Step 4: Go to clients page, check the client is connected to satellite")
    public void step4() {
        WebCheck.checkHrefIcon(URLParam.hrefClients);
        MyCommonAPIs.sleepi(2);
        SelenideElement satelliteclient = $x("//span[text()='PRJCBUGEN-T25342']/../../td[1]//span");
        SelenideElement satellitemodel = $x("//span[text()='PRJCBUGEN-T25342']/../../td[4]");
        assertTrue(satelliteclient.getText().equals(WebportalParam.client2name), "checkpoint 4 : check client name");
        assertTrue(satellitemodel.getText().equals(WebportalParam.ob2sate1Model), "checkpoint 5 : check satellite model");
    }
