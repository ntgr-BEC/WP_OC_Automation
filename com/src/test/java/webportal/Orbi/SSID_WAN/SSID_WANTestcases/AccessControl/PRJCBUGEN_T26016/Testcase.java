package webportal.Orbi.SSID_WAN.SSID_WANTestcases.AccessControl.PRJCBUGEN_T26016;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T26016") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify whether user able to create an Allow access rule for same Clients for two different routers in same VPN group") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T26016") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: Go to Routers page / User add allow rule on central router and remote router for client2 in access control page")
    public void step2() {
        boolean checkpoint;
        RoutersBusinessVPNPage page = new RoutersBusinessVPNPage();
        page.gotoPage();
        ...
        page.clickNext();
        page.setPage3WirelessSettings(true, "PRJCBUGEN-T26016", 1, "12345678");
        page.clickNext();
        ...
        page.cirbtnAdd.click();
        page.setPage4AccessControl(WebportalParam.client2name, WebportalParam.client2wifimac, 0, WebportalParam.ob2deveiceName);
    ...
    @Step("Test Step 3: Check client2 cannot ping central router")
    public void step3() {
        ...
        for(int i=0;i<3;i++) {
            ...
            cmd = String.format("WAFconnectBSSID PRJCBUGEN-T26016 12345678 WPA2PSK aes %s", WebportalParam.ob1SSIDWANbssid);
            if (!new Javasocket()
            ...
            assertTrue(checkpoint, "checkpoint 1 : client2 can connect to central router");
            util.MyCommonAPIs.sleep(10000);
        ...
        assertTrue(checkpoint, "checkpoint 2 : client2 cannot ping central router");
    }
    
    @Step("Test Step 4: Check client2 cannot ping to remote router")
    public void step4() {
        boolean checkpoint = false;
        String cmd;
        for(int i=0;i<3;i++) {
            ...
            util.MyCommonAPIs.sleep(10000);
            // connect to bssid (remote router's)
            cmd = String.format("WAFconnectBSSID PRJCBUGEN-T26016 12345678 WPA2PSK aes %s", WebportalParam.ob2SSIDWANbssid);
            if (!new Javasocket()
                    .sendCommandToWinClient(WebportalParam.client2ip, WebportalParam.client2port, cmd)
                    .equals("true")) {
                checkpoint = false;
            ...
            System.out.println(checkpoint);
            assertTrue(checkpoint, "checkpoint 3 : client2 can connect to remote router");
            util.MyCommonAPIs.sleep(10000);
        ...
        assertTrue(checkpoint, "checkpoint 4 : client2 cannot ping remote router");
    }
