package webportal.Orbi.SSID_WAN.SanityBusinessVPNFlow.PRJCBUGEN_T26877;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T26877") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify whether Wireless clients connected to Outdoor RBS50Y satellite are able to access the VPN") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T26877") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 3: Remote client can connect to RBS50Y")
    public void step3() {
        boolean checkpoint = false;
        String cmd;
        String satellite_bssid = "";
        // check satellite1 and satellite2 contains RBS50Y
        if(WebportalParam.ob2sate1Model.equals("RBS50Y")) {
            satellite_bssid = WebportalParam.ob2sate1SSIDWANbssid;
        }
        else if(WebportalParam.ob2sate2Model.equals("RBS50Y")) {
            satellite_bssid = WebportalParam.ob2sate2SSIDWANbssid;
        }
        assertFalse(satellite_bssid.equals(""), "checkpoint 1 : RBS50Y is in config");
