package webportal.OrganizationWideSSID.CreateORGWideSSIDBeforeAddingAP.PRJCBUGEN_T32711;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T32711") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("verify org-wide-ssid-custom vlan funcationally.") // It's                                                                                                                                                     // Jira
    // Test
    // Case.
    @TmsLink("PRJCBUGEN_T32711") // It's a testcase id/link from Jira Test Case.
    ...
    public void step1() {
      ...
      public void step4() {
          ...
          assertTrue(new OrganizationPage(false).checkOrgSsidCustomVlanIsExist("PRJCBUGEN_T32711"),"ssid with Custom VLAN does not exits in Loc 1");
      ...
      public void step5() {
          ...
          MyCommonAPIs.sleepi(2);
          assertTrue(new APUtils(WebportalParam.ap1IPaddress).getSsidStatus1("PRJCBUGEN_T32711", WebportalParam.ap1Model),"ssid is not pushed to AP");
          MyCommonAPIs.sleepi(10);
          assertTrue(new APUtils(WebportalParam.ap2IPaddress).getSsidStatus1("PRJCBUGEN_T32711", WebportalParam.ap2Model),"ssid is not pushed to AP");
          MyCommonAPIs.sleepi(10);
          int sum = 0;
          while (true) {
              MyCommonAPIs.sleepi(10);
              if (new Javasocket()
                      .sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "WAFfindSSID PRJCBUGEN_T32711")
                      .indexOf("true") != -1) {
                  break;
              } else if (sum > 30) {
          ...
          if (!new Javasocket()
                  .sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "WAFconnect PRJCBUGEN_T32711 123456798 WPA2PSK aes")
                  .equals("true")) {
              result1 = false;
              if (new Javasocket()
                      .sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "WAFconnect PRJCBUGEN_T32711 123456798 WPA2PSK aes")
