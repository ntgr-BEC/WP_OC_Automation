package webportal.OrganizationWideSSIDEnhancement.EnterpriseOrgWideSSIDAPNotNeeded.PRJCBUGEN_T35299;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T35299") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Verify WPA2 Enterprise SSID and WPA3 Enterprise SSID with 2 bands is pushed to newly created locations after disabling enabling Organization wide SSID") // It's                                                                                                                                                     // Jira
    // Test
    // Case.
    @TmsLink("PRJCBUGEN_T35299") // It's a testcase id/link from Jira Test Case.
    ...
    public void step1() {
      ...
      public void step3() {
          ...
          orgssidInfo.put("SSID", "PRJCBUGEN_T35299");
          assertTrue(new OrganizationPage(false).verifylocation1WPA3EnterpriseOrgSSID2Bands(orgssidInfo)," org wide ssid with wpa3 enterprise security not pushed successfull.");  
          ...
          new OrganizationPage(false).openOrg(WebportalParam.Organizations);  
          new OrganizationPage(false).goToOrgSsid(WebportalParam.Organizations);
          new OrganizationPage(false).disableOrgWideSSIDSwitch();
      }
      
      @Step("Test Step4:Verify that after disabling org wide ssid switch existing org wide ssid's should not be pushed to newly created locations")
      public void step4() {
          ...
          locationInfo.put("Country", "United States of America");
          List<String> Loclist=new ArrayList<String>();  
          int totalLocationsrequired = 4;
          int created = 2;
          for(int i = 3; i <=totalLocationsrequired; i++){
              Random random = new Random();
              int num = random.nextInt(100000);
              String formatted = String.format("%05d", num);
              Loclist.add(locationName+i);
          ...
          new OrganizationPage(false).goWirelessSetting();
          assertTrue(new OrganizationPage(false).verfiyNoDataAvailableonWirelestLocation()," after disabling org wide ssid switch existing org wide ssid's is pushed to newly created location");  
          
          new OrganizationPage(false).openOrg(WebportalParam.Organizations);
          new MyCommonAPIs().gotoLoction("office4");
          new OrganizationPage(false).goWirelessSetting();
          assertTrue(new OrganizationPage(false).verfiyNoDataAvailableonWirelestLocation(), "after disabling org wide ssid switch existing org wide ssid's is pushed to newly created location");
          
      }
      
      @Step("Test Step5:Verify that after enabling org wide ssid switch existing org wide ssid's should be pushed to newly created locations ")
      public void step5() {
          
          new OrganizationPage(false).openOrg(WebportalParam.Organizations);  
          new OrganizationPage(false).goToOrgSsid(WebportalParam.Organizations);
          new OrganizationPage(false).enableOrgWideSSIDSwitch();
