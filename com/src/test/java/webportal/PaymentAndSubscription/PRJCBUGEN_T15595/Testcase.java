package webportal.PaymentAndSubscription.PRJCBUGEN_T15595;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T15595") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("check the \"Cancel\" button functionality") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T15595") // It's a testcase id/link from Jira Test Case.
    ...
    public void step1() {
        ...
        accountInfo.put("Last Name", "T15595");
    ...
    @Step("Test Step 2: Buy premium subscription, then cancel subscription;")
    public void step2() {
        ...
        paymentInfo.put("Last Name", "T15595");
        ...
        if (new HamburgerMenuPage(false).checkSubscriptionScreen(paymentInfo.get("Number of Device Credits"))) {
            ...
            result1 = new HamburgerMenuPage(false).checkSubscriptionScreenForBasic();
            if (new HamburgerMenuPage().checkAccountTryTrial()) {
                new HamburgerMenuPage(false).upgradeSubscription(paymentInfo);
                if (new HamburgerMenuPage(false).checkSubscriptionScreen(paymentInfo.get("Number of Device Credits"))) {
                    new HamburgerMenuPage().cancelSubscription();
                    result2 = new HamburgerMenuPage(false).checkCancelSubscription();
                }
            }
        }
        assertTrue(result1 && result2, "Subscription status is incorrect.");
    }
