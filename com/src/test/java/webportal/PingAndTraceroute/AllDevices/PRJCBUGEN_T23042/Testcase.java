package webportal.PingAndTraceroute.AllDevices.PRJCBUGEN_T23042;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T23042") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("verify, traceroute test can be performed at location level for multiple device") // It's a testcase title from Jira Test Case.verify, traceroute test can be performed at location level for multiple device
    @TmsLink("PRJCBUGEN-T23042") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        CompleteResultOrbi = troubleshoot.CheckTracerouteResult(WebportalParam.sw1serialNo);
        CompleteResultswitch = troubleshoot.CheckTracerouteResult(WebportalParam.ob1serialNo);
        ...
        assertTrue(CompleteResultOrbi.contains(WebportalParam.getLocText("traceroute to www.bing.com").toLowerCase()), "ping lookup result is not complete.");
        assertTrue(CompleteResultswitch.contains(WebportalParam.getLocText("traceroute to www.bing.com").toLowerCase()), "ping lookup result is not complete.");
