package webportal.PingAndTraceroute.Premium.PRJCBUGEN_T23053;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T23053") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("verify, ping test can run continuously") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T23053") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 3: Check ping test again option;")
    public void step3() {   
        
        tmpStr = troubleshoot.runPingTestAgain(pingTraceroute,domain, WebportalParam.ap1serialNo);
        CompleteResult = troubleshoot.CheckPingResult(WebportalParam.ap1serialNo);
        System.out.println(tmpStr);
        assertTrue(tmpStr.contains(WebportalParam.getLocText("Success").toLowerCase()), "ping  result is not Success.");
        assertTrue(CompleteResult.contains(WebportalParam.getLocText("3 packets transmitted").toLowerCase()), "ping  result is not complete.");
    ...
    @Step("Test Step 4: Check ping test again option;")
    public void step4() {   
