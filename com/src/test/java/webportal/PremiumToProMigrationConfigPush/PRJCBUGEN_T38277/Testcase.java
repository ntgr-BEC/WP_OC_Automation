package webportal.PremiumToProMigrationConfigPush.PRJCBUGEN_T38277;
...
public class Testcase extends TestCaseBase {
    ...
    String              organizationName = "PRJCBUGEN_T38277";
    ...
    @Story("PRJCBUGEN_T38277") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to verify that premium account having All bands WPA3 Enterprise SSID with connected client migrated to pro account and verify same config is there or not")                                                                                                                                                             // Case.
    @TmsLink("PRJCBUGEN_T38277") // It's a testcase id/link from Jira Test Case.
    ...
    public void step4() {
        ...
        ssidInfo.put("SSID", "PRJCBUGEN_T38277");
        ssidInfo.put("Security", "WPA3 Enterprise");
        ...
        new Javasocket().sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "sudo nmcli device wifi list | grep -F \"PRJCBUGEN_T38277\"");
        MyCommonAPIs.sleepi(20);
        new Javasocket().sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "sudo -S nmcli c add type wifi con-name \"PRJCBUGEN_T38277\" ifname wlp4s0 ssid \"PRJCBUGEN_T38277\"");      
        MyCommonAPIs.sleepi(20);
        new Javasocket().sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "sudo -S nmcli connection modify \"PRJCBUGEN_T38277\" wifi-sec.key-mgmt wpa-eap 802-1x.eap peap 802-1x.phase2-auth mschapv2 802-1x.identity \"teju\" 802-1x.password \"teju\"");
        MyCommonAPIs.sleepi(20);
        new Javasocket().sendCommandToWinClient(WebportalParam.clientip, WebportalParam.clientport, "sudo -S nmcli connection up \"PRJCBUGEN_T38277\"");
    ...
    public void step6() {
        ...
        assertTrue(new HamburgerMenuPage(false).addLocationsToOrg(organizationName),
                "Location is not Successfully added to new created orgnizqation");
        new WirelessQuickViewPage().verifyOrganizationWirelessQuickViewPage("PRJCBUGEN_T38277");
    }
