package webportal.ProManagedSwitch.ACL.PRJCBUGEN_T41317;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T41317") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("086-[Network Setup] Customer allow policy: Edit To") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T41317") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 2: Create customer IP allow policy")
    public void step2() {
        netsp.camData.fromname = "name1";
        netsp.camData.fromip = "100.1.1.1";
        netsp.camData.fromipmask = "0.0.0.255";
        netsp.camData.toname = "name2";
        netsp.camData.toip = "200.1.1.1";
    ...
    @Step("Test Step 4: Edit To info under network setup")
    public void step4() {
        netsp.gotoPage();
        netsp.openNetwork(networkName);
        netsp.gotoStep(2);
        netsp.openACL(netsp.camData.fromip, true);
        netsp.camData.toname = "testcase033";
        tmpStr1 = netsp.camData.toip;
        netsp.camData.toip = "10.10.10.10";
    ...
    public void step5() {
        ...
        tmpStr = String.format("permit %s %s %s %s", netsp.camData.fromip, netsp.camData.fromipmask, netsp.camData.toip, netsp.camData.toipmask);
        assertTrue(SwitchCLIUtils.ACLClass.aclResult.contains(tmpStr), "verify permit ip host: " + tmpStr);
        tmpStr = String.format("permit %s %s %s %s", netsp.camData.fromip, netsp.camData.fromipmask, tmpStr1, netsp.camData.toipmask);
