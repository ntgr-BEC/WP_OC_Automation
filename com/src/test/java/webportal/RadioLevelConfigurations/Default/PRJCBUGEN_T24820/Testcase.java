package webportal.RadioLevelConfigurations.Default.PRJCBUGEN_T24820;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T24820") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Enable/ Disable control for radio status  and  wireless modes  are  present  or  not  at  device  level for 3 radio  AP ") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T24820") // It's a testcase id/link from Jira Test Case.
...
//        new DevicesDashPage().checkDeviceInAdminAccount();
    ...
    public void step2() {
        ...
        MyCommonAPIs.sleepi(10);
        assertTrue(new WirelessQuickViewPage(false).Radiomode5high.isDisplayed() && new WirelessQuickViewPage(false).Channel5high.isDisplayed() 
                && new WirelessQuickViewPage(false).ChannelWidth5high.isDisplayed() && new WirelessQuickViewPage(false).OutputPower5high.isDisplayed() 
                && new WirelessQuickViewPage(false).enableradio5high.isDisplayed(), "All options are not present under AP");
