package webportal.ScreenNavigationProAcct.PRJCBUGEN_T34397;
...
public class Testcase extends TestCaseBase {
    ...
    @Feature("Screen Navigation ProAcct IM-7.0") // It's a folder/component name to make test suite more readable from Jira
                                                 // Test Case.
    @Story("PRJCBUGEN_T34397") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("Test to Verify Device Settings page navigation in the pro Account") // It's a testcase
                                                                                      // title from Jira
    // Test Case.
    @TmsLink("PRJCBUGEN-T34397") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 4: Onboarding a dummy device;")
    public void step4() {
    ...
    @Step("Test Step 5: Go to Org Setting")
    public void step5() {
        new OrganizationPage().openOrg(organizationName);
        new OrganizationPage(false).GoToOrgSetting();
        new OrganizationPage(false).GoToDeviceSectionUnderSetting();
    }

    @Step("Test Step 6:verify All Data are showing correct on Device Dashboard")
    public void step6() {
        assertTrue(new OrganizationPage(false).verifyDeviceDashboardHaveCorrectData(WebportalParam.ap5serialNo), "Device data is showing wrong");
    }
}
