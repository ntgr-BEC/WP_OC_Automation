package webportal.Switch.VLAN.PRJCBUGEN_T4713;
...
//        MyCommonAPIs.sleep(8000);
    ...
    public void step4() {
        ...
        if (result2.contains("Voice VLAN") || (result2.contains("Auto-VoIP"))) {
            micResult = true;
        } else {
            micResult = false;
...
//        String result1 = SwitchCLIUtils.getVoiceVlan();
        if (result2.contains("voice-vlan") &&  (result2.contains("voip"))) {
            micResult = true;
        }else if(result2.contains("auto-voip protocol-based") &&  (result2.contains("auto-voip oui-based"))) {
            micResult = true;
        }
        else {
