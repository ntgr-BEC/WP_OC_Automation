package webportal.Switch.VLAN.PRJCBUGEN_T4716;
...
public class Testcase extends TestCaseBase implements Config {
    ...
    @Story("PRJCBUGEN_T4716") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("004-Create a vlan via the template of \"Data Network Configuration/Custom Network\"") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T4716") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        vlanPage.addDataVlanWithPorts("data traffic vlan", "600", dut1Name, sw1port, "tag", null, null, null, null);
    ...
    public void step4() {
        ...
        for (int i = 0; i < sw1port.length; i++) {
            String result1 = MyCommonAPIs
                    ...
                    .toLowerCase();
            if (SwitchCLIUtils.isTagPort("g" + sw1port[i], vlanId) && result1.contains("600")) {
                micResult = true;
            } else {
                micResult = false;
                assertTrue(micResult, "----Check Point 2 Fail:check vlan 600 fail, cli is:" + result1);
            }
        }
    }

    @Step("Test Step 5: delete vlan 600")
    public void step5() {
        WiredQuickViewPage wiredQuickViewPage = new WiredQuickViewPage();
        WiredVLANPageForVLANPage vlanPage = new WiredVLANPageForVLANPage();
        vlanPage.deleteVlan("data traffic vlan");
        MyCommonAPIs.sleep(8000);
    }
