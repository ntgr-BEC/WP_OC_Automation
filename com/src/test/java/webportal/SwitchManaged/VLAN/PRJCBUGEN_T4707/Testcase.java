...
public class Testcase extends TestCaseBase implements Config {
    ...
    String         sMaxVlan = null;
    String         vlanId   = "200";

    @Feature("Switch.VLAN") // It's a folder/component name to make test suite more readable from Jira Test Case.
    @Story("PRJCBUGEN_T4707") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description("010-Create and delete maximum vlans") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T4707") // It's a testcase id/link from Jira Test Case.
    @Test(alwaysRun = true, groups = "p5") // "p3"
    @Issue("For gs limit is 64")
    ...
    public void step3() {
        ...
        if (SwitchCLIUtilsMNG.isTagPort(port1, "200") && result1.contains("200")) {
            micResult = true;
        } else {
            micResult = false;
        ...
        if (SwitchCLIUtilsMNG.isTagPort(port2, "200") && result1.contains("200")) {
            micResult = true;
        } else {
            micResult = false;
