package webportal.TargetFirmware.PRJCBUGEN_T40372;
...
public class Testcase extends TestCaseBase {
    ...
    @Story("PRJCBUGEN_T40372") // It's a testcase id/link from Jira Test Case but replace - with _.
    @Description(" Test to set the TARGET Firmware and check whether the target FW is reflected in the firmware page [UI verification] ") // It's a testcase title from Jira Test Case.
    @TmsLink("PRJCBUGEN-T40372") // It's a testcase id/link from Jira Test Case.
    ...
    public void step2() {
        ...
        if(!FWversion.contains(WebportalParam.ap1Firmware)) {
            new APUtils(WebportalParam.ap1IPaddress).switchTheFW();
        }
    
    }
    
    @Step("Test Step 3: Set the target firmware and check whether the target FW is reflected on firmware page  ;")
    public void step3() {
        
        new FirmwarePage().gotoFirmwarePage();  
        assertTrue(new FirmwarePage().TargetFWonFWPage(), "Th set TARGET FW is not reflecting on firmware page");
