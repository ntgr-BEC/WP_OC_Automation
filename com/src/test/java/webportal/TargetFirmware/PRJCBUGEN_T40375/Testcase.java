package webportal.TargetFirmware.PRJCBUGEN_T40375;
...
public class Testcase extends TestCaseBase {
    ...
    @Description("Test to select a target firwmare and then update the FW by SCHEDULE UPDATE option") // It's a testcase title from Jira Test Case.
...
//        }
    ...
    @Step("Test Step 3: Set the target firmware adn then update the FW by SCHEDUKLE UPDATE option  ;")
    public void step3() {
        
        new FirmwarePage().gotoFirmwarePage(); 
        new FirmwarePage().setTargetFW();
        
        if (fmp.waitForUpdateAvailable() && fmp.Schedule(false)) {
            System.out.println("device is upgraded via cloud to the set TARGET FIRMWARE");
        }
        new RunCommand().enableSSH4APALL(WebportalParam.ap1IPaddress);
        String FWversion= new APUtils(WebportalParam.ap1IPaddress).getVersion(false);
        assertTrue((FWversion.contains(WebportalParam.ap1TargetFirmware) || (new WirelessQuickViewPage().FWverOnInsight.getText().equals(WebportalParam.ap1TargetFirmware))), "The AP is not updated to the TARGET GFIRMWARE");
