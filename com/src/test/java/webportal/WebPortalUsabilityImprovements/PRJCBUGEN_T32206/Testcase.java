package webportal.WebPortalUsabilityImprovements.PRJCBUGEN_T32206;
...
public class Testcase extends TestCaseBase {
    ...
    @Description("Test to verify that user should able to see all org when click on “View All” link on the bottom of the pop-up.") // It's a
                                                                                                                                   // testcase title
                                                                                                                                   // from Jira Test
                                                                                                                                   // Case.
    @TmsLink("PRJCBUGEN-T32206") // It's a testcase id/link from Jira Test Case.
    ...
    @Step("Test Step 5: Verify that user should be able to see the organization after clicking View All Button ")
    public void step5() {
        new ManagerPage(false).clickOnViewAllButton(mailname);
        assertTrue(new ManagerPage(false).VerifyOrgIsPresentOnViewAllSection(organizationName1), "Org is not Present on that page");
    }
}
